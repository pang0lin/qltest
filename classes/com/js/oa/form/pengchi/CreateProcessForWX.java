  ...
  public List<Map<String, String>> getFieldValueForWX() {
    ...
    try {
      ...
      if (ukids.length() > 0) {
        ...
        pstmt.executeBatch();
        conn.commit();
        System.out.println("------成功修改WX单的读取状态--------------");
    ...
    if (wx != null && wx.size() > 0 && wxmx != null && wxmx.size() > 0) {
      ...
      for (Map<String, String> wxmap : wx) {
        ...
        if (!result.equals("-1") && result.length() < 10) {
          System.out.println("时间：[" + printTime + "]创建采购单WX成功！--单号是：" + (String)wxmap.get("ddh"));
          continue;
        } 
        if (result.equals("-1")) {
          System.out.println("时间：[" + printTime + "]创建采购单WX失败！--单号是：" + (String)wxmap.get("ddh"));
