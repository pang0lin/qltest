...
public final class item_005fselect_005fact_005f3g_jsp extends org.apache.jasper.runtime.HttpJspBase
...
if("1".equals(fromSelUser)){
	...
	if(null!=request.getParameter("status") && "0".equals(request.getParameter("status"))){
		...
		if(!"".equals(newSelectedOper)){
			operName = newSelectedOper.split(",")[1];
			operId = newSelectedOper.split(",")[0];
		}
	}
	
	if(!operId.startsWith(",") && !operId.startsWith("$")) operId = ","+operId;
	//if(!operId.endsWith(",") && !operId.endsWith("$")) operId = operId + ",";
	String[] ids = operId.replace(",,", ";").replace(",", "").split(";");
	String[] names = operName.split(",");
	operName = "";
	operId = "";
	for(int i=0;i<ids.length;i++){
		if(!operId.contains(","+ids[i]+",")){
			operId += ","+ids[i]+",";
			operName += names[i]+",";
		}
	}
	IO2File.printFile(operName+"   对应Id：    "+operId);
...
if(null == userAgent){
	userAgent = "";
      ...
      out.print(path );
      out.write("/wap2/workitem/item_info_3g.jsp");
      out.print(showDoc );
      ...
      out.write("\">\r\n\t\r\n\t<div class=\"f_1\">处理待办</div>\r\n    <div class=\"box_2\">\r\n    \r\n    ");
if("3".equals(currentActivityClass)){//自动返回节点 
      ...
      out.write("\r\n\t\t\t\t<textarea id=\"include_comment\" name=\"include_comment\" rows=\"2\" maxlength=\"1000\">");
      out.print(include_comment );
      out.write("</textarea>\r\n\t\t\t\t</div>\r\n\t\t\t\t<div class=\"clear\"></div>\r\n\t\t</div>\r\n\t\t<div class=\"list_2\">\r\n\t\t\t\t<div class=\"t1\">&nbsp;</div>\r\n\t\t\t\t<div class=\"t2\">\r\n\t\t\t\t<input class=\"button2\" type=\"button\" value=\"提交\" onclick=\"javascript:submitForm();\"/>\t\t\r\n\t\t\t\t</div>\r\n\t\t\t\t<div class=\"clear\"></div>\r\n\t\t</div>\r\n\t\t");
}else{
	    ...
	    ConcurrentHashMap activityMap = null; 
		if(!activityClass.equals("0")){
			...
			//判断是否设置自动审批，自动审批节点在没有办理人的情况下可以跳过 2015-01-15
	    	ProcessActivityUtil actUtil=new ProcessActivityUtil();
	    	activityMap=actUtil.getNextActivityInfo(request,curActivityId,tableId,recordId);
	    	if(activityMap.get("nextActivityVO")==null){
	    		activityVO = new com.js.oa.jsflow.util.ProcessStep().getProceedNextActi(activityClass, tableId, recordId, request);
	    	}else{
	    		activityVO=(ActivityVO)activityMap.get("nextActivityVO");
	    	}
      ...
      out.write("\"/>\r\n\t\t\t\t</div>\r\n\t\t\t</div>\t\r\n\t\t\r\n\t\t");
			...
			String approveModet = "";
			String fapm = "1"; 
      out.write("\t\r\n\t\t\t<div class=\"list_2\">\r\n\t\t\t\t<div class=\"t1\">下一节点:</div>\r\n\t\t\t\t<div class=\"t2\">\r\n\t\t\t\t\t<select name=\"nextActivity\" id=\"nextActivity\" onchange=\"selectChange(this)\">\r\n\t\t\t\t\t");
for(int i=0;i<activityList.size();i++){ 
						activityVO=(ActivityVO)activityList.get(i);
						if(i==0){
							fapm = activityVO.getTransactType();
						}
						...
						approveModet += activityVO.getId()+":"+activityVO.getTransactType()+";";
						activityName=activityVO.getName()==null?"":activityVO.getName(); 
      out.write("\r\n\t\t\t\t\t\t<option value=\"");
      out.print(activityVO.getId());
      out.write(',');
...
if(mainNextActivityId.equals(String.valueOf(activityVO.getId()))){out.print("selected");} 
      ...
      out.write("\r\n\t\t           </select>\r\n\t\t\t\t</div>\r\n\t\t\t\t<input type=\"hidden\" name=\"approveMode\" id=\"approveMode\" value=\"");
      out.print(fapm );
      out.write("\" />\r\n\t\t\t\t<input type=\"hidden\" id=\"approveModeTemp\" value=\"");
      out.print(approveModet );
      out.write("\" />\r\n\t\t\t\t<div class=\"clear\"></div>\r\n\t\t\t</div>\r\n\t\t\t");
} //end if(list size==1) 
      out.write("\r\n\t\t    <div class=\"list_2\" ");
...
if(!isComplete){
      ...
      out.print(operName );
      out.write("\"/>\r\n\t\t\t\t<input class=\"button2\" type=\"button\" onclick=\"javascript:selectEmp();\" value=\"选择\"/>\r\n\t\t\t\t</div>\r\n\t\t\t\t<div class=\"clear\"></div>\r\n\t\t\t</div>\r\n\t\t");
}//end if(isComplete);
		}//end if list == null	
      out.write("\r\n\t    \r\n\t    <div class=\"list_2\">\r\n\t\t\t<div class=\"t1\">&nbsp;</div>\r\n\t\t\t<div class=\"t2\">\r\n\t\r\n\t\t\t");
if(isComplete){
      out.write("\r\n\t\t\t<input class=\"button4\" type=\"button\" value=\"办理结束\" onclick=\"javascript:endForm();\"/>\r\n\t\t\t");
}else{ 
      out.write("\t\r\n\t\t\t<input class=\"button2\" type=\"button\" value=\"提交\" onclick=\"javascript:submitForm();\"/>\r\n\t\t\t");
}
      out.write("\r\n\t\t\t</div>\r\n\t\t\t<div class=\"clear\"></div>\r\n\t\t</div>\r\n\t");
}//end of "3".equals(currentActivityClass) 
      out.write("\r\n\t</div>\r\n\t<div class=\"height1\"></div>\r\n  </div>\r\n</form>\r\n\r\n</div>\r\n</body>\r\n</html>\r\n<script>\r\nfunction submitForm(){\r\n\tdocument.frm.action=\"");
      out.print(path);
      out.write("/wap2/workitem/item_submit_3g.jsp\";\r\n\t\r\n\tif(document.all.operId.value==\"\" && \"3\"!=document.all.activityClass.value){\r\n\t\talert(\"请选择办理人！\");\r\n\t}else if(document.all.opinionMust.value==\"1\" && document.all.include_comment.value==\"\"){\r\n\t\talert(\"请填写批示意见！\");\t\t\r\n\t}else{\r\n\t\tdocument.frm.submit();\r\n\t}\r\n}\r\nfunction selectEmp(){\r\n\tdocument.frm.action=\"");
      out.print(path);
      out.write("/wap2/workitem/item_select_emp_3g.jsp\";\r\n\tdocument.frm.submit();\r\n}\r\nfunction endForm(){\r\n\tdocument.frm.action=\"");
      out.print(path);
      out.write("/wap2/workitem/item_submit_complete_3g.jsp\";\r\n\tif(document.all.opinionMust.value==\"1\" && document.all.include_comment.value==\"\"){\r\n\t\talert(\"请填写批示意见！\");\t\t\r\n\t}else{\r\n\t\tdocument.frm.submit();\r\n\t}\r\n}\r\nfunction selectChange(obj){\r\n\tvar arr = new Array();\r\n\tvar approveModeTemp = document.getElementById(\"approveModeTemp\").value.split(\";\");\r\n\tfor(var i=0;i<approveModeTemp.length;i++){\r\n\t\tvar a = approveModeTemp[i].split(\":\");\r\n\t\tarr[a[0]]=a[1];\r\n\t}\r\n\tvar v = obj.value.split(\",\");\r\n\tdocument.getElementById(\"approveMode\").value = arr[v[0]];\r\n}\r\nfunction changeComment(){\r\n\tvar content=document.getElementById(\"commentSelect\").value;\r\n\tdocument.getElementById(\"include_comment\").value=content;\r\n}\r\n</script>");
