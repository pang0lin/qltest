...
public class ReceiveFileEJBBean extends HibernateBase implements SessionBean {
  ...
  public Long save(GovReceiveFilePO po) throws Exception {
    ...
    try {
      if (po.getId() > 0L) {
        Long xxxid = Long.valueOf(po.getId());
  ...
  public GovReceiveFilePO get(String receiveFileId) throws Exception {
    GovReceiveFilePO result = null;
    begin();
    try {
      result = (GovReceiveFilePO)this.session.get(GovReceiveFilePO.class, new Long(receiveFileId));
    } catch (Exception e) {
      System.out.println("---------------------------------------------");
      e.printStackTrace();
      System.out.println("---------------------------------------------");
  ...
  public Integer delete(String id) throws Exception {
    ...
    try {
      ...
      rs.close();
      if ("0".equals(tableId)) {
        rs = stmt.executeQuery("select t.sendfile_tableid from doc_documentsendfile t where t.DOCUMENTSENDFILE_ID=" + id);
        if (rs.next())
          tableId = rs.getString(1); 
        rs.close();
      } 
  ...
  public Integer delete2(String id) throws Exception {
    Integer result = Integer.valueOf("0");
    Connection conn = null;
    try {
      conn = (new DataSourceBase()).getDataSource().getConnection();
