 ...
 * Generated at: 2023-01-06 10:06:48 UTC
...
public final class wf_005ftran_005forg_jsp extends org.apache.jasper.runtime.HttpJspBase
...
if(!"0".equals(processType)){
	...
	if("0".equals(tranType)){
		...
		if(nextUser[0]!=null && nextUser[0].equals("3")){
			//指定全部办理人
			java.util.List candidate = workFlowBD.getCandidate(nextUser[1],nextUser[2]);
		...
		if(nextUser[0]!=null && nextUser[0].equals("15")){
			//从岗位中指定 
			java.util.List candidate = workFlowBD.getPositionUserIDAndName(nextUser[18], request.getParameter("submitPersonId"));
			for(int i=0;i<candidate.size();i++){
				Object[] tmp = (Object[])candidate.get(i); 
				selectUser += tmp[0]+",";
				selectUserName += tmp[1]+",";
...
//判断页面显示方式
if(participantType==1 || participantType==8 || participantType==2 || participantType==3 || participantType==4 || participantType==9 || participantType==10 || participantType==6 || participantType==13 || participantType==0 || participantType==15 || participantType==7 || participantType==11 || "0".equals(tranType) || "1".equals(tranType) || "2".equals(tranType)){
