...
public class CreateProcessForONOP {
  ...
  public List<Map<String, String>> getFieldValueForOP() {
    ...
    try {
      ...
      if (ukids.length() > 0) {
        ...
        pstmt.executeBatch();
        conn.commit();
        System.out.println("------成功修改OP(东莞)单的读取状态----时间：[" + sdf.format(new Date()) + "]");
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("ONOPDG"))
      ...
      for (Map<String, String> opmap : op) {
        ...
        if (judgeForm(opmx).equals("ONOP"))
          try {
            ...
            xml.append("<Data>");
            if (fqr.equals(info.get("submitNameZjb"))) {
              xml.append("<Table tableId=\"").append(info.get("tableIdZjb")).append("\" tableName=\"").append(info.get("tableName")).append("\">");
            } else {
            ...
            for (Map<String, String> mxmap : opmx) {
              if (((String)opmap.get("ukzj")).equals(mxmap.get("ukzj")) && ((String)opmap.get("ddlx")).equals(mxmap.get("ddlx")) && ((String)opmap.get("ddgs")).equals(mxmap.get("ddgs")) && ((String)opmap.get("ddh")).equals(mxmap.get("ddh")) && ((String)opmap.get("ddhz")).equals(mxmap.get("ddhz"))) {
                ...
                xml.append("<field name=\"jst_3430_f6349\" type=\"varchar\">").append(((String)mxmap.get("ckj")).trim()).append("</field>");
            ...
            if (flowxml.indexOf("SubTable") > 0) {
              ...
              if (!result.equals("-1") && result.length() < 10) {
                System.out.println("时间：[" + printTime + "]创建采购单ONOP（东莞）成功！--单号是：" + (String)opmap.get("ddh"));
                continue;
              } 
              if (result.equals("-1")) {
                System.out.println("时间：[" + printTime + "]创建采购单ONOP（东莞）失败！--单号是：" + (String)opmap.get("ddh"));
  ...
  public String judgeForm(List<Map<String, String>> mx) {
    ...
    for (Map<String, String> mxmap : mx) {
      if (((String)mxmap.get("mxlx")).equals(""))
        num++; 
    } 
    if (num == mx.size())
      flag = "ONOP"; 
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("ONOPDG")) {
          ...
          nodeMap.put("tableIdZjb", node.getAttributeValue("tableIdZjb"));
