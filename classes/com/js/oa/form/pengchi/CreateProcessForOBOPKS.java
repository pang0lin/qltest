...
public class CreateProcessForOBOPKS {
  ...
  public List<Map<String, String>> getFieldvalueForOPmx(String ukid) {
    List<Map<String, String>> datas = new ArrayList<Map<String, String>>();
    String sql = "select KDUKID,KDDCTO,KDKCOO,KDDOCO,KDOCTO,KDSFXO,KDLITM,KDMCU,KDAN8,KDTRDJ,KDVR01,KDDSC1,KDDSC2,DRDL01,KDPRP3,KDUOM,KDUOM2,KDPRRC,KDSQOR,KDUORG,KDURAT,CRPDTA.NTOD(KDDRQJ) KDDRQJ from PRODDTA.FE64311D@jdedblink,PRODCTL.F0005@jdedblink where  DRSY='41' and DRRT='P3' and DRKY=KDPRP3 and KDPOHP01=0 and KDPOHP02='N' and KDKCOO='00016' and KDOCTO='OB' and KDUKID='" + 
      ukid + "'";
    DataSource ds = (new DataSourceBase()).getDataSource("jdbc/pengchi");
    Connection conn = null;
    try {
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("OBOPKS"))
          ...
          try {
            ...
            for (Map<String, String> mxmap : opmx) {
              if (((String)opmap.get("ukzj")).equals(mxmap.get("ukzj")) && ((String)opmap.get("ddlx")).equals(mxmap.get("ddlx")) && ((String)opmap.get("ddgs")).equals(mxmap.get("ddgs")) && ((String)opmap.get("ddh")).equals(mxmap.get("ddh")) && ((String)opmap.get("ddhz")).equals(mxmap.get("ddhz"))) {
                ...
                xml.append("<field name=\"jst_3394_f6354\" type=\"varchar\">").append(((String)mxmap.get("ckj")).trim()).append("</field>");
            ...
            if (flowxml.indexOf("SubTable") > 0) {
              ...
              if (!result.equals("-1") && result.length() < 10) {
                System.out.println("时间：[" + printTime + "]创建采购单OBOP（昆山）成功！--单号是：" + (String)opmap.get("ddh"));
                continue;
              } 
              if (result.equals("-1")) {
                System.out.println("时间：[" + printTime + "]创建采购单OP（昆山）失败！--单号是：" + (String)opmap.get("ddh"));
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("OBOPKS")) {
