...
public class WorkflowForOG extends Workflow {
  public String complete(HttpServletRequest request) {
    String result = super.complete(request);
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
    String recordId = request.getParameter("recordId");
    String comment = request.getParameter("include_comment");
    String sql = "Select ukzj,shjg,ddgs,ddh,ddlx,ddhz From kf_sxd Where kf_sxd_id=?";
    ...
    String sql1 = "Select ukzj From kf_sxdmx Where kf_sxdmx_foreignkey=?";
    ...
    try {
      ...
      pstmt2.close();
      if (resultInt > 0) {
        System.out.println("-----中间表头数据(OG)-东莞回写成功---UKID:【" + ukIdForOg + "】");
      } else {
        System.out.println("-----中间表头数据(OG)-东莞回写失败---UKID:【" + ukIdForOg + "】");
      } 
  ...
  public void back(HttpServletRequest request) {
    ...
    String sql = "Select ukzj,ddgs,ddh,ddlx,ddhz From kf_sxd Where kf_sxd_id=?";
    DataSourceBase db = new DataSourceBase();
    DataSource ds = db.getDataSource();
    Connection conn = null;
    PreparedStatement pstmt = null;
    ...
    try {
      ...
      rs.close();
      pstmt.close();
      String thsql = "update  kf_sxd set thbz=? where kf_sxd_id=?";
    ...
    try {
      ...
      pstmt2.close();
      if (resultInt > 0) {
        System.out.println("-----中间表头(OG)-东莞数据退回：回写成功-订单号：OG【" + ogddh + "】");
        if (ukIdForOgmx.size() > 0) {
        ...
        System.out.println("-----中间表头(OG)-东莞数据退回：回写失败-订单号：OG【" + ogddh + "】");
      } 
      conn2.close();
    } catch (Exception e) {
      if (conn2 != null)
