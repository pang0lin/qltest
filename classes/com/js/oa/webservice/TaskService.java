...
public class TaskService {
  ...
  public String getGonggao(String key, String userAccounts) {
    String result = SecurityUtil.checkSecurity(key, "gonggaoService");
  ...
  public String getGonggaoCount(String key, String userAccounts) {
    String result = SecurityUtil.checkSecurity(key, "gonggaoService");
    String userId = StaticParam.getEmpIdByAccount(userAccounts);
    StringBuffer xmlStr = new StringBuffer();
    if ("".equals(result) && !"".equals(userId)) {
      ...
      for (int j = 1; j < orgIds.length; j++)
        orgSql = String.valueOf(orgSql) + " i.informationreaderorg like '%*" + orgIds[j] + "*%' or"; 
      String sqlAll = "select count(i.information_id) from oa_information i join oa_informationChannel c on i.channel_id=c.channel_id where i.channel_id in (" + 
        channelIds + ") and ((i.informationreader like '%$" + userId + "$%' or (" + orgSql + " i.informationreaderorg like '%*-1*%') " + 
        "or(CONCAT(i.informationreaderorg,i.informationreader)='' or CONCAT(i.informationreaderorg,i.informationreader) is null)))";
      String sqlToRead = "select count(i.information_id) from oa_information i join oa_informationChannel c on i.channel_id=c.channel_id where i.channel_id in (" + 
        channelIds + ") and ((i.informationreader like '%$" + userId + "$%' or (" + orgSql + " i.informationreaderorg like '%*-1*%') " + 
        "or(CONCAT(i.informationreaderorg,i.informationreader)='' or CONCAT(i.informationreaderorg,i.informationreader) is null)))" + 
        " and i.information_id not in (select information_id from oa_informationbrowser where emp_id=" + userId + ")";
      int allCount = (new DataSourceUtil()).getQueryCount(sqlAll);
      int toReadCount = (new DataSourceUtil()).getQueryCount(sqlToRead);
      xmlStr.append("<?xml version='1.0' encoding='UTF-8'?>");
      xmlStr.append("<result>");
      xmlStr.append("<code>0</code>");
      xmlStr.append("<allCount>" + allCount + "</allCount>");
      xmlStr.append("<toReadCount>" + toReadCount + "</toReadCount>");
      xmlStr.append("<moreUrl>" + moreUrl + "</moreUrl>");
      xmlStr.append("</result>");
    } else {
      xmlStr.append(result);
    } 
  ...
  public String getDaiban(String key, String userAccounts) {
    String result = SecurityUtil.checkSecurity(key, "taskService");
    StringBuffer xmlStr = new StringBuffer();
    userAccounts = (new UserBD()).getUserAccountsByEnglistName(userAccounts);
    String userId = StaticParam.getEmpIdByAccount(userAccounts);
    if ("".equals(result)) {
      if (!"".equals(userId)) {
        int num = 15;
        ...
        result = "<?xml version='1.0' encoding='UTF-8'?><result><code>0</code><resultSet><errmsg>该用户不存在</errmsg></resultSet></result>";
        xmlStr.append(result);
      } 
    } else {
      xmlStr.append(result);
  ...
  public String getTaskNum(String key, String userAccounts) {
    ...
    if ("".equals(result)) {
      if (!"".equals(userId)) {
        ...
        result = "<?xml version='1.0' encoding='UTF-8'?><result><code>0</code><errmsg>该用户不存在</errmsg></result>";
        xmlStr.append(result);
      } 
    } else {
      xmlStr.append(result);
