...
try{ 	   
    ...
    response.setHeader("Content-disposition", "attachment; filename=receivefile.xls");// 设定输出文件头   
    ...
    //创建一个工作表    
    jxl.write.WritableSheet ws = wwb.createSheet("收文", 10);
    ws.setRowView(0, 500);
    ws.setColumnView(0, 15);//设置第一列宽度
    ...
    ws.addCell(new jxl.write.Label(0, 0, "流水号", wcf));
	ws.addCell(new jxl.write.Label(1, 0, "来文单位", wcf));
	ws.addCell(new jxl.write.Label(2, 0, "来文日期", wcf));
	ws.addCell(new jxl.write.Label(3, 0, "文号", wcf));
	ws.addCell(new jxl.write.Label(4, 0, "标题", wcf));
	ws.addCell(new jxl.write.Label(5, 0, "办理状态", wcf));
	ws.addCell(new jxl.write.Label(6, 0, "办理时限", wcf));
