...
public class WorkflowForONOP extends Workflow {
  public String complete(HttpServletRequest request) {
    String result = super.complete(request);
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
    String recordId = request.getParameter("recordId");
    String comment = request.getParameter("include_comment");
    String sql = "Select ukzj,ddgs,ddh,ddlx,ddhz From jst_3429 Where jst_3429_id=?";
  ...
  public void back(HttpServletRequest request) {
    ...
    try {
      ...
      rs.close();
      pstmt.close();
      String thsql = "update  jst_3429 set thbz=? where jst_3429_id=?";
    ...
    String sql1 = "Select ukzj From jst_3430 Where jst_3430_foreignkey=?";
    List<String> ukIdForOpmx = new ArrayList<String>();
    Connection conn1 = null;
    PreparedStatement pstmt1 = null;
    ResultSet rs1 = null;
    ...
    try {
      ...
      pstmt2.close();
      if (resultInt > 0) {
        System.out.println("-----中间表头(OP)数据退回：回写成功--订单号:ONOP【" + opddh + "】");
      } else {
        System.out.println("-----中间表头(OP)数据退回：回写失败--订单号:ONOP【" + opddh + "】");
      } 
      conn2.close();
    } catch (Exception e) {
      if (conn2 != null)
