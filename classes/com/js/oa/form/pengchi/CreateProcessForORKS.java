  ...
  public List<Map<String, String>> getFieldValueForOR() {
    ...
    try {
      ...
      if (ukids.length() > 0) {
        ...
        pstmt.executeBatch();
        conn.commit();
        System.out.println("------成功修改OR（昆山）单的读取状态-------时间：[" + sdf.format(new Date()) + "]");
    ...
    if (or != null && or.size() > 0 && ormx != null && ormx.size() > 0) {
      ...
      for (Map<String, String> ormap : or) {
        ...
        if (!result.equals("-1") && result.length() < 10) {
          System.out.println("时间:[" + printTime + "]创建采购单OR(昆山)成功！--单号是：" + (String)ormap.get("ddh"));
          continue;
        } 
        if (result.equals("-1")) {
          System.out.println("时间:[" + printTime + "]创建采购单OR(昆山)失败！--单号是：" + (String)ormap.get("ddh"));
