...
public final class wf_005fgetnextactivity_jsp extends org.apache.jasper.runtime.HttpJspBase
...
if("0".equals(directSend)){
	...
	String firstActIsBranch=request.getAttribute("firstActIsBranch")==null?"0":request.getAttribute("firstActIsBranch").toString();
	if(!"0".equals(processType)){
		if("getFirstActivity".equals(type)){
			if(!"1".equals(firstActIsBranch)){
				...
				//第一个虚拟节点的节点ID
				activityIds=request.getAttribute("firstCurActivityId").toString();
			}
		}else{
			/*中间节点*/
			if(request.getAttribute("activityIds")!=null && !"".equals(request.getAttribute("activityIds").toString())){
	...
	if("end".equals(transactType)){	// 后续节点没有办理人时，结束流程
      ...
      out.write("\r\n\t\t\t<script type=\"text/javascript\">\r\n\t\t\twindow.parent.cmdSend2('firstActIsBranch;");
      out.print(activityIds);
      out.write("');\t\r\n\t\t\t</script>\r\n\t\t");
	...
	//若下一节点只有一个节点且是固定办理人，则直接发送
	//下一步骤办理人
	String selUser=request.getAttribute("selectUser")==null?"":request.getAttribute("selectUser").toString();
	String passroundUser = request.getAttribute("passroundUser")==null?"":request.getAttribute("passroundUser").toString();
