...
public final class pengform_005fsetfieldvalue_jsp extends org.apache.jasper.runtime.HttpJspBase
...
if (flag.equals("wl")){//主表查询-根据物料编码（料号），查询物料的名称、规格型号
	sql="select TRIM(IMDSC1),TRIM(IMDSC2) from PRODDTA.f4101 WHERE TRIM(IMLITM)='"+para+"'";	
}else if(flag.equals("wlcz")){//主表查询-根据物料编码（料号），查询物料的名称、规格型号、材质，（可查物料材质信息，若无，则显示空）
	sql="select TRIM(IMDSC1),TRIM(IMDSC2), TRIM(DRDL01) from PRODDTA.f4101 left join PRODCTL.F0005 on TRIM(IMSRP0)=TRIM(DRKY) AND DRSY='41' AND DRRT='10' where TRIM(IMLITM)='"+para+"'";	
}else if(flag.equals("ryqj")){
	sql="select TRIM(ABALPH) from PRODDTA.f0101 where ABAN8='"+para+"'";
}
//sql="select IMLITM,IMDSC1,IMDSC2, DRDL01 from PRODDTA.f4101 LEFT JOIN PRODCTL.F0005 ON TRIM(IMSRP0)=TRIM(DRKY) AND DRSY='41' AND DRRT='10' and trim(IMLITM)=";
