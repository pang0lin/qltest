...
public class WorkflowForORKS extends Workflow {
  public String complete(HttpServletRequest request) {
    String result = super.complete(request);
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
    String recordId = request.getParameter("recordId");
    String comment = request.getParameter("include_comment");
    String sql = "Select ukzj,shjg,ddgs,ddh,ddlx,ddhz From jst_3462  Where jst_3462_id=?";
    ...
    String sql1 = "Select ukzj From jst_3461  Where jst_3461_foreignkey=?";
    ...
    try {
      ...
      pstmt2.close();
      if (resultInt > 0) {
        System.out.println("-----中间表头(OR)数据审批回写成功（昆山）------UKID：【" + ukIdForOq + "】");
        ...
        System.out.println("-----中间表头(OR)数据审批回写失败（昆山）------UKID：【" + ukIdForOq + "】");
      } 
      conn2.close();
    } catch (Exception e) {
      if (conn2 != null)
  ...
  public void back(HttpServletRequest request) {
    ...
    String sql = "Select ukzj,ddgs,ddh,ddlx,ddhz From jst_3462 Where jst_3462_id=?";
    DataSourceBase db = new DataSourceBase();
    DataSource ds = db.getDataSource();
    Connection conn = null;
    PreparedStatement pstmt = null;
    ...
    try {
      ...
      rs.close();
      pstmt.close();
      String thsql = "update  jst_3462 set thbz=? where jst_3462_id=?";
    ...
    try {
      ...
      pstmt2.close();
      if (resultInt > 0) {
        System.out.println("-----中间表头(OR)数据退回-昆山：回写成功--订单号：OR【" + oqddh + "】");
        if (ukIdForOqmx.size() > 0) {
        ...
        System.out.println("-----中间表头(OR)数据退回-昆山：回写失败--订单号：OR【" + oqddh + "】");
      } 
      conn2.close();
    } catch (Exception e) {
      if (conn2 != null)
