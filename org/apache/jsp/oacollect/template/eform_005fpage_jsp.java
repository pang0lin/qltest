...
public final class eform_005fpage_jsp extends org.apache.jasper.runtime.HttpJspBase
  ...
  static {
    ...
    _jspx_dependants.put("/oacollect/template/showform.jsp", Long.valueOf(1499751428000L));
  }
...
if(request.getParameter("close") != null  ){
      ...
      out.write("\";\r\n\tif(f==\"collectList\"){\r\n\t\twindow.close();\r\n\t\topener.location.href=\"/jsoa/collectDealwithAction.do?action=getPage&fromFlag=");
      out.print(fromFlag);
     ...
     if(request.getParameter("processId")!=null){
          ...
          //取读字段
        	...
        	if("0".equals(request.getParameter("workStatus"))){
        	    //是待办文件
        	    //取当前节点的可写字段List
              java.util.List list=(java.util.List)request.getAttribute("RWFieldList");
              String[] str = null;
          ...
          for (int i = 0; i < tables.length; i++) {
	          ...
	          if(data!=null && data.length>0){
	                    ...
	                    for(int n=0;n<data.length;n++){//遍历数据，解析数据在表单上的展示
	                            //解析数据
	                            for(int m=0;m<forFields.length;m++){//遍历字段，生成该字段及其值展示在表单上的脚本
	                                out.print(cst.getForeignEditHTML(forFields[m][5],data[n][m],forFields,foreignHideField,m,String.valueOf(n),forFields[m][0],parent_fromDraft,tables[i],request.getAttribute("infoId").toString(),data[n][forFields.length],forFields[m][10],allCoumputFieldStr));
	                            }
	                            String script = "document.getElementsByName('"+forFields[0][0]+"-"+forFields[0][5]+"')["+String.valueOf(n)+"].innerHTML+='<input type=hidden id="+tables[i]+"id name="+tables[i]+"id value="+data[n][forFields.length]+">';";
	                           out.print(script);
	                    }
...
/*
 ...
 if (flagg == null || flagg.equals("0")) {
      out.write("\r\n\t\t<input type=\"button\" id=\"saveAndExit\" class=\"btnButton4font\" onclick=\"javascript:if(customFormBeforeSubmit()){fnSaveAndExit();}\" value=\"保存退出\" />\r\n\t\t<input type=\"button\" id=\"saveAndContinue\" class=\"btnButton4font\" onclick=\"javascript:save();\" style=\"display:'");
      out.print(flag);
