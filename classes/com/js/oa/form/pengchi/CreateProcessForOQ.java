...
public class CreateProcessForOQ {
  ...
  public List<Map<String, String>> getFieldValueForOQ() {
    ...
    String sql = "select KHUKID,KHDCTO,KHKCOO,KHDOCO,KHSFXO,KHORBY,KHDL09,KHVR01,KHRMK from PRODDTA.FE64311H@jdedblink where KHPOHP01=0 and KHNXTR=' ' and KHDCTO= 'OQ' and KHKCOO='00010' ";
    ...
    try {
      ...
      if (ukids.length() > 0) {
        ...
        pstmt.executeBatch();
        conn.commit();
        System.out.println("------成功修改OQ（东莞）单的读取状态-------时间：[" + sdf.format(new Date()) + "]");
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("OQDG"))
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("OQDG")) {
