...
public class CreateProcessForONOPKS {
  ...
  public List<Map<String, String>> getFieldValueForOP() {
    ...
    try {
      ...
      if (ukids.length() > 0) {
        ...
        pstmt.executeBatch();
        conn.commit();
        System.out.println("------成功修改OP（昆山）单的读取状态-------时间：[" + sdf.format(new Date()) + "]");
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("ONOPKS"))
          ...
          try {
            ...
            for (Map<String, String> mxmap : opmx) {
              if (((String)opmap.get("ukzj")).equals(mxmap.get("ukzj")) && ((String)opmap.get("ddlx")).equals(mxmap.get("ddlx")) && ((String)opmap.get("ddgs")).equals(mxmap.get("ddgs")) && ((String)opmap.get("ddh")).equals(mxmap.get("ddh")) && ((String)opmap.get("ddhz")).equals(mxmap.get("ddhz"))) {
                ...
                xml.append("<field name=\"jst_3437_f6348\" type=\"varchar\">").append(((String)mxmap.get("ckj")).trim()).append("</field>");
            ...
            if (flowxml.indexOf("SubTable") > 0) {
              ...
              if (!result.equals("-1") && result.length() < 10) {
                System.out.println("时间：[" + printTime + "]创建采购单ONOP（昆山）成功！--单号是：" + (String)opmap.get("ddh"));
                continue;
              } 
              if (result.equals("-1")) {
                System.out.println("时间：[" + printTime + "]创建采购单ONOP（昆山）失败！--单号是：" + (String)opmap.get("ddh"));
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("ONOPKS")) {
