...
public final class receivefile_005fdetail_005flist_jsp extends org.apache.jasper.runtime.HttpJspBase
    implements org.apache.jasper.runtime.JspSourceDependent,
                 org.apache.jasper.runtime.JspSourceImports {


/** 获取两个时间段之间的天数（扣除周六周日）**/
private int getDays(String spectTime,String doneTime){
	int res=0;
	...
	int[] weekDay={-1,-1,-1}; 
	try{
		java.util.Date begin=format.parse(spectTime);
		java.util.Date end=format.parse(doneTime);
		
		Calendar calEnd=Calendar.getInstance();
		calEnd.setTime(end);
		Calendar calBegin=Calendar.getInstance();
		calBegin.setTime(simpleformat.parse((simpleformat.format(begin))));
		...
		while(calBegin.before(calEnd)){
			...
			/** 如果办结期限是周五 如果周末(含周六、周日)办理的 超期一天，跨周末后周一办理的也算超期一天 **/
						
			if(week==7){
				//周六的情况
				res++;
			}else if(week==1){
				//周日，判断周六是否加过一天，如果已经加过则忽略周日
				if(weekDay[1]!=7){
					//前一天未记录周六，说明周日是第一次计算
					res++;
				}else{
...
int index = 0 ;
	  ...
	  String receiveDate,expectDoneTime,doneTime;
            ...
            try {
              ...
              // /chart/receivefile/receivefile_detail_list.jsp(145,3) name = id type = java.lang.String reqTime = false required = true fragment = false deferredValue = false expectedTypeName = null deferredMethod = false methodSignature = null
              _jspx_th_logic_005fiterate_005f0.setId("list");
              // /chart/receivefile/receivefile_detail_list.jsp(145,3) name = name type = null reqTime = true required = false fragment = false deferredValue = false expectedTypeName = null deferredMethod = false methodSignature = null
              _jspx_th_logic_005fiterate_005f0.setName("list");
              // /chart/receivefile/receivefile_detail_list.jsp(145,3) name = scope type = null reqTime = true required = false fragment = false deferredValue = false expectedTypeName = null deferredMethod = false methodSignature = null
		...
		receiveDate=obj[3]!=null?obj[3].toString():"";
		if(receiveDate.indexOf(" ")>0){
			receiveDate=receiveDate.substring(0,receiveDate.indexOf(" "));
		}
		expectDoneTime=obj[4]!=null?obj[4].toString():"";
		doneTime=obj[5]!=null?obj[5].toString():"";
                  ...
                  out.write('"');
                  out.write('>');
                  out.print(getDays(expectDoneTime,doneTime));
                  out.write("&nbsp;</td>\r\n\t  </tr>\r\n\t  ");
