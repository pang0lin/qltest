...
public class CreateProcessForOG {
  ...
  public List<Map<String, String>> getFieldValueForOG() {
    ...
    try {
      ...
      if (ukids.length() > 0) {
        ...
        pstmt.executeBatch();
        conn.commit();
        System.out.println("------成功修改OG（东莞）单的读取状态--------------");
  ...
  public List<Map<String, String>> getFieldvalueForOGmx() {
    List<Map<String, String>> datas = new ArrayList<Map<String, String>>();
    String sql = "select PDASID,PDDL04,PDDS50,PDUOM,PDDL03,PDPRRC,PDUORG,PDDSPR,PDURAT,PDVR01,CRPDTA.NTOD(PDDRQJ) PDDRQJ,CRPDTA.NTOD(PDPDDJ) PDPDDJ,PDSGTXT,PDD200,PDDL02,PDUKID,PDDCTO,PDKCOO,PDDOCO,PDSFXO from PRODDTA.FE64319D@jdedblink where PDPOHP01=0 and PDKCOO='00010' and PDDCTO=?";
    DataSource ds = (new DataSourceBase()).getDataSource("jdbc/pengchi");
    Connection conn = null;
    try {
      conn = ds.getConnection();
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("OGDG"))
    ...
    if (og != null && og.size() > 0 && ogmx != null && ogmx.size() > 0) {
      ...
      for (Map<String, String> ogmap : og) {
        ...
        if (!result.equals("-1") && result.length() < 10) {
          System.out.println("时间：+[" + printTime + "] 创建送修单OG（东莞）成功！--单号是：" + (String)ogmap.get("ddh"));
          continue;
        } 
        if (result.equals("-1")) {
          System.out.println("时间：+[" + printTime + "]创建送修单OG（东莞）失败！--单号是：" + (String)ogmap.get("ddh"));
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("OGDG")) {
