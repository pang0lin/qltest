...
public class EventAction extends Action {
  public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws Exception {
    ...
    if ("addEvent".equals(action)) {
      ...
      eventVO.setOpenEvent((httpServletRequest.getParameter("openEvent") == null) ? 
          "1" : httpServletRequest.getParameter("openEvent"));
    ...
    if ("getMonthView".equals(action)) {
      ...
      String eventContent = httpServletRequest.getParameter("eventContent");
      monthData = getDDEvents(httpServletRequest, userId, domainId, evenTitle, eventContent);
    ...
    if ("underlingemp".equals(action)) {
      ...
      List<Object[]> empList = worklogBD.getDownEmployeeList(userId.toString());
      ...
      if ("day".equals(type)) {
        ...
        if (suserId != null && !"".equals(suserId))
          if ("0".equals(suserId)) {
            ...
            List<Object[]> UnderList = userBD.selectMyUnderling(String.valueOf(userId));
            if (!UnderList.isEmpty())
              for (int i = 0; i < UnderList.size(); i++) {
                Object[] obj = UnderList.get(i);
                list.addAll(eventBD.getEventByDay(Long.valueOf(obj[0].toString()), newDate, domainId, eventTitle, eventContent));
              }  
          } else {
            list = eventBD.getEventByDay(Long.valueOf(suserId), newDate, domainId, eventTitle, eventContent);
          }  
        Calendar prev = Calendar.getInstance();
        prev.setTime(strToDate(nowdate));
        prev.add(5, -1);
        ...
        httpServletRequest.setAttribute("evenTitle", eventTitle);
        httpServletRequest.setAttribute("eventContent", eventContent);
        return actionMapping.findForward("goto_AllEventDayUderling");
      } 
      if ("getMonthView".equals(type)) {
        ...
        DayData dayData = null;
        List monthData = new ArrayList();
        cal.setTimeInMillis((new Date(cal.get(1) - 1900, cal.get(2), 
              cal.get(5))).getTime());
        String evenTitle = httpServletRequest.getParameter("eventTitle");
        String eventContent = httpServletRequest.getParameter("eventContent");
        if (suserId != null && !"".equals(suserId))
          monthData = getDDEvents(httpServletRequest, 
              Long.valueOf(suserId), domainId, evenTitle, eventContent); 
        httpServletRequest.setAttribute("monthData", monthData);
        httpServletRequest.setAttribute("Year", year);
        httpServletRequest.setAttribute("Month", month);
        httpServletRequest.setAttribute("Day", day);
        ...
        httpServletRequest.setAttribute("evenTitle", evenTitle);
        httpServletRequest.setAttribute("eventContent", eventContent);
        return actionMapping.findForward("goto_TheMonthEventUderling");
      } 
      if ("getThisWeek".equals(type)) {
              ...
              if ("0".equals(suserId)) {
                if (!UnderList.isEmpty())
                  for (int j = 0; j < UnderList.size(); j++) {
                    ...
                    list.addAll(eventBD.getEventByDay(Long.valueOf(obj[0].toString()), new Long(dayData.getDay()), domainId, eventTitle, eventContent));
                  }  
              } else {
                list = eventBD.getEventByDay(Long.valueOf(suserId), new Long(dayData.getDay()), domainId, eventTitle, eventContent);
              }  
            dayData.setEvents(list);
            weekData.add(dayData);
            httpServletRequest.setAttribute("logDay" + (i + 1), 
        ...
        httpServletRequest.setAttribute("evenTitle", eventTitle);
        httpServletRequest.setAttribute("eventContent", eventContent);
        return actionMapping.findForward("goto_ThisAllWeekEventUnderling");
      } 
      if ("selectAllEvent".equals(type)) {
        ...
        List list = new ArrayList();
        String userIds = "-1";
        if (suserId != null && !"".equals(suserId) && suserId != "0") {
          if (eventType == null || "".equals(eventType) || 
            "null".equals(eventType))
            eventType = null; 
          long start_time = 0L;
          ...
          if (!"1".equals(isDate)) {
            if (empName == null || empName.equals("") || empName.equals("null")) {
              list = eventBD.searchEvent(Long.valueOf(suserId), 
                  ...
                  null, null, null, null, eventType, eventTitle);
            } else {
              UserBD userDB = new UserBD();
              List<EmployeeVO> userList = userDB.getUserIdByEmpName(empName);
              StringBuffer sb = new StringBuffer();
              if (userList != null && userList.size() > 0)
                for (int i = 0; i < userList.size(); i++) {
                  EmployeeVO ev = userList.get(i);
                  String evId = String.valueOf(ev.getEmpId());
              ...
              if (sb.toString() != null && !sb.toString().equals("")) {
                ...
                list = eventBD.searchEventByEmpName(ids, 
                    new Integer(currentPage), eventContent, domainId, 
                    null, null, null, null, eventType, eventTitle, empName);
              } else {
          ...
          list = eventBD.selectAllEmpEvent(userIds, new Integer(currentPage), domainId, eventTitle, eventContent);
        } 
        String recordCount = String.valueOf(eventBD.getRecordCount());
        httpServletRequest.setAttribute("allEventList", list);
        httpServletRequest.setAttribute("recordCount", recordCount);
        ...
        httpServletRequest.setAttribute("evenTitle", eventTitle);
        httpServletRequest.setAttribute("eventContent", eventContent);
        return actionMapping.findForward("goto_AllEventUnderling");
      } 
    } 
  ...
  private List getDDEvents(HttpServletRequest httpServletRequest, Long userId, Long domainId, String evenTitle, String eventContent) {
    String day = httpServletRequest.getParameter("Day");
    String month = httpServletRequest.getParameter("Month");
    String year = httpServletRequest.getParameter("Year");
    if (year == null || "".equals(year)) {
    ...
    if (userId.longValue() == 0L) {
      ...
      try {
        ...
        List<Object[]> UnderList = userBD.selectMyUnderling(String.valueOf(object));
        if (!UnderList.isEmpty())
          for (int i = 0; i < UnderList.size(); i++) {
            Object[] obj = UnderList.get(i);
            List result = bd.getDDEvents(Long.valueOf(obj[0].toString()), beginDate, endDate, domainId, evenTitle, eventContent);
            Calendar cal3 = (Calendar)cal2.clone();
            setDDEvents(monthData, result, cal3, showAllDate);
          }  
      } catch (Exception e) {
        e.printStackTrace();
      } 
    } else {
      List result = bd.getDDEvents(userId, beginDate, endDate, domainId, evenTitle, eventContent);
      setDDEvents(monthData, result, cal2, showAllDate);
    } 
    return monthData;
  }
  
  private void setDDEvents(List<DayData> datas, List<EventVO> result, Calendar cal3, int days) {
    long time = 0L;
    ...
    for (int i = 0; i < days; i++) {
      dayData = new DayData();
      list = new ArrayList();
      time = cal3.getTimeInMillis();
      Date currentDate = cal3.getTime();
      int y = cal3.get(1);
      int m = cal3.get(2);
      int d = cal3.get(5);
      int w = cal3.get(7);
      ...
      if (result != null && result.size() > 0)
        for (int j = 0; j < result.size(); j++) {
          ...
          if (vo.getOnTimeMode().intValue() == 0) {
            if (vo.getEventBeginDate().longValue() <= time && 
              vo.getEventEndDate().longValue() >= time)
              list.add(vo); 
            continue;
          } 
          if (vo.getOnTimeMode().intValue() == 1) {
          ...
          if (vo.getOnTimeMode().intValue() == 2) {
            if (vo.getEchoMode().intValue() == -1) {
              if (vo.getEchoBeginTime().compareTo(
                  currentDate) <= 0)
          ...
          if (vo.getOnTimeMode().intValue() == 3) {
            if (vo.getEchoMode().intValue() == -1) {
              if (vo.getEchoBeginTime().compareTo(
                  currentDate) <= 0)
                if (Integer.parseInt(vo.getOnTimeContent()) == 
                  d) {
                  list.add(vo);
                  continue;
                }  
          ...
          if (vo.getOnTimeMode().intValue() == 4) {
            String[] monthDay = vo.getOnTimeContent().split(
                "\\$");
            if (vo.getEchoMode().intValue() == -1) {
              if (vo.getEchoBeginTime().compareTo(
                  currentDate) <= 0)
                if (Integer.parseInt(monthDay[0]) == m + 1 && 
