...
try{
	selValue=com.js.util.util.CharacterTool.unescape(selValue);
}catch(Exception e){}
...
if(submitPerson!=null&&!"".equalsIgnoreCase(submitPerson)){
	submitPerson=com.js.util.util.CharacterTool.unescape(submitPerson);
}
...
<root>
   <data>      
      <formfield></formfield>
      <fetchdata>0</fetchdata>  0:ֱ�Ӹ��ֶθ�ֵ  1:������������Ҫ id��text
      <content></content>
   </data>
   <data>      
      <formfield></formfield>
      <fetchdata>1</fetchdata>  0:ֱ�Ӹ��ֶθ�ֵ  1:�������������Ҫ�����ֶ�(id��text)
      <content>
         <id></id>
         <text></text>
      </content>
   ...
   <data>      
   <formfield></formfield>
   <fetchdata>2</fetchdata>   0:ֱ�Ӹ��ֶθ�ֵ  1:�������������Ҫ�����ֶ�(id��text) 2:��Ҫ���ֵ һ��Ϊִ��sql��ȡ����
   <content>
   	  <subdata>
   	    <id></id>
        <text></text>
...
try{
	...
	String fetchSql="",toFormField="",fieldValue="", fieldTable = "";
	String[][] fetchSqlPara;
	
	java.sql.PreparedStatement pstmt=conn.prepareStatement("select field_fetchsql,field_toffield,field_value,field_table from tfield where field_id=?");
	...
	if(toFormField!=null && toFormField.length()>0){
		...
		if(fetchSql!=null && fetchSql.toLowerCase().indexOf("select ")>=0 && fetchSql.toLowerCase().indexOf(" from ")>0){
			...
			fetchSql=fetchSql.replaceAll("\\@\\$\\@submitPerson\\@\\$\\@",submitPerson);
			fetchSql=fetchSql.replaceAll("\\@\\$\\@curYear\\@\\$\\@", (new Date().getYear()+1900) + "");
			fetchSql=fetchSql.replaceAll("\\@\\$\\@curMonth\\@\\$\\@", (new Date().getMonth()+1) + "");
			
			String[] fetchSqlArr=fetchSql.split("\\@\\@");
			for(int fi=0;fi<fetchSqlArr.length;fi++){
				if(!"".equals(fetchSqlArr[fi]) && fetchSqlArr[fi].indexOf(" ")<0){					
					fetchSql=fetchSql.replaceAll("\\@\\@"+fetchSqlArr[fi]+"\\@\\@",request.getParameter(fetchSqlArr[fi])==null?"":request.getParameter(fetchSqlArr[fi]));
				}
			}
		...
		for(i=0;i<fetchSqlPara.length;i++){
			...
			// ��ѯҪ��ֵ�Ŀؼ�������
			rs = stmt.executeQuery("select field_show from tfield where field_name='" + fetchSqlPara[i][0] + "' and field_table=" + fieldTable);
			if(rs.next()){
				buffer.append("<fieldtype>").append(rs.getString(1)).append("</fieldtype>");
			}
