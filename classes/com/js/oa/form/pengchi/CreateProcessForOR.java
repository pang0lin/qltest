...
public class CreateProcessForOR {
  ...
  public List<Map<String, String>> getFieldValueForOR() {
    ...
    String sql = "select KHUKID,KHDCTO,KHKCOO,KHDOCO,KHSFXO,KHORBY,KHDL09,KHVR01,KHRMK from PRODDTA.FE64311H@jdedblink  where KHPOHP01=0 and KHNXTR=' ' and KHDCTO= 'OR' and KHKCOO='00010' ";
    ...
    try {
      ...
      if (ukids.length() > 0) {
        ...
        pstmt.executeBatch();
        conn.commit();
        System.out.println("------成功修改OR（东莞）单的读取状态-------时间：[" + sdf.format(new Date()) + "]");
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("ORDG"))
    ...
    if (or != null && or.size() > 0 && ormx != null && ormx.size() > 0) {
      ...
      for (Map<String, String> ormap : or) {
        ...
        if (!result.equals("-1") && result.length() < 10) {
          System.out.println("时间:[" + printTime + "]创建采购单OR(东莞)成功！--单号是：" + (String)ormap.get("ddh"));
          continue;
        } 
        if (result.equals("-1")) {
          System.out.println("时间:[" + printTime + "]创建采购单OR(东莞)失败！--单号是：" + (String)ormap.get("ddh"));
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("ORDG")) {
