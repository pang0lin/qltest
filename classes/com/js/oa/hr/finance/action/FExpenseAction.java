...
public class FExpenseAction extends Action {
  private static final String masCityService = null;
  
  public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
    HttpSession session = request.getSession(true);
    String action = request.getParameter("action");
    FExpenseActionForm form = (FExpenseActionForm)actionForm;
    String userId = session.getAttribute("userId").toString();
    String userName = session.getAttribute("userName").toString();
    String orgId = session.getAttribute("orgId").toString();
    String orgIdString = session.getAttribute("orgIdString").toString();
    FExpenseService services = new FExpenseService();
      ...
      try {
        ...
        if (financeSalaryCheckPassWord != null && "0".equals(financeSalaryCheckPassWord)) {
          ...
          if ((new Date()).before(date)) {
            ...
            request.setAttribute("javaAction", "fExpenseAction");
