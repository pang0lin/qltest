...
public final class reportList_005f3g_jsp extends org.apache.jasper.runtime.HttpJspBase
...
if(reportlist!=null&&reportlist.size()>0){ 
	...
	int length = 0;
		for(int i=0;i<reportlist.size();i++){
			...
			empName = "mine".equals(repUser)?"":empName;
			if(null!=logDate)dateStr = new SimpleDateFormat("MM-dd").format(logDate);	
			conHead = po.getLogContent();
			
			if(null!=conHead && !"".equals(conHead)){
				length = conHead.length();
				conHead = length>15?conHead.substring(0,15)+"...":conHead;
			}
...
dateStr = "";
	...
	if(recordCount>WapUtil.LIMITED){
		if(upIndex>=0)upUrl=path+"/wap/action/WapReportAction.do?method=getWapRepList&amp;beginIndex=" + upIndex + "&amp;repUser=" + repUser;
		if(nextIndex<recordCount)nextUrl=path+"/wap/action/WapReportAction.do?method=getWapRepList&amp;beginIndex=" + nextIndex+ "&amp;repUser=" + repUser;
      out.write("\r\n\t\t <DIV class=page>\r\n       ");
if(upUrl.indexOf("#")<0){
