 ...
 * Generated at: 2023-01-06 10:08:36 UTC
...
public final class OfficeServer_jsp extends org.apache.jasper.runtime.HttpJspBase
...
public class iWebOffice {
 ...
 private String isdm = SystemCommon.getIsdm();
...
// ************* Bolb字段处理、公共函数、接收流、写回流代码    结束  *******************************
 ...
 //调出文档，将文档内容保存在mFileBody里，以便进行打包
          ...
          if (result.next()) {
            try {				
                ...
                mResult = true ;				
				   ...
				   mFileBody = InfoUtil.read_oracle_blob("Document_File", "FileBody", "RecordID", Long.valueOf(mRecordID));
				   mResult = true ;
				}else{
  ...
  private boolean LoadFileFromDisk(String fileName,String path)
  {
    boolean mResult = false;
    mFileBody=null;
    File f = new File(path+"/upload/govdocumentmanager_his/"+fileName);   
    if(!f.exists()){
    	f=new File(path+"/upload/govdocumentmanager/"+fileName);
    	if(!f.exists()){ 
        	return false;
    	}
    } 
    try{
       mFileBodyDM =getBytesFromFile(f);  
    }catch(Exception e){
       e.printStackTrace();
    }
      ...
      if (DbaObj.OpenConnection()) {
        ...
        try {
		 ...
		 if ("oracle".equals(databaseType)&&"0".equals(isdm)) {
          ...
          DbaObj.Conn.commit();
          prestmt.close();
          InfoUtil.insert_oracle_blob("Document_File", "FileBody", "Fileid", (long)iFileId, mFileBody);
          mFileBody=null;
          mResult=true;
	   }else if("mysql".equals(databaseType)){
          ...
          if (result.next()) {
            try {
				...
				if ("oracle".equals(databaseType)&&"0".equals(isdm)) {
			      ...
			      mFileBody = InfoUtil.read_oracle_blob("Template_File", "FileBody", "RecordID", Long.valueOf(mTemplate));
			      mResult = true ;
			   }else if("mysql".equals(databaseType)){
        ...
        try {
            ...
            if ("oracle".equals(databaseType)&&"0".equals(isdm)) {
			      ...
			      prestmt =DbaObj.Conn.prepareStatement(Sql);
					  ...
					  DbaObj.Conn.commit();
					  prestmt.close();
					  InfoUtil.insert_oracle_blob("Template_File", "FileBody", "TEMPLATEID", (long)mTemplateId, mFileBody);
					  mFileBody=null;
					  mResult=true;
			   }else if("mysql".equals(databaseType)){
        ...
        try {
          ...
          while (result.next()) {
            try {
                ...
                if ("oracle".equals(databaseType)&&"0".equals(isdm)) {// mysql , sqlserver 的 没写
          ...
          if (result.next()) {
            try {        
			   ...
			   if ("oracle".equals(databaseType)&&"0".equals(isdm)) {
			      ...
			      mFileBody = InfoUtil.read_oracle_blob("Version_File", "FileBody", "RecordID", Long.valueOf(mFileID));
			      mResult = true ;
			   }else if("mysql".equals(databaseType)){
        ...
        try {
		    ...
		    if ("oracle".equals(databaseType)&&"0".equals(isdm)) {
			      ...
			      prestmt =DbaObj.Conn.prepareStatement(Sql);
					  ...
					  DbaObj.Conn.commit();
					  prestmt.close();
					  InfoUtil.insert_oracle_blob("Version_File", "FileBody", "Fileid", (long)iFileId, mFileBody);
					  mFileBody=null;
					  mResult=true;
			   }else if("mysql".equals(databaseType)){
    ...
    if (request.getMethod().equalsIgnoreCase("POST")){
      ...
      if (MsgObj.GetMsgByName("DBSTEP").equalsIgnoreCase("DBSTEP")){	//如果是合法的信息包
        ...
        if(mOption.equalsIgnoreCase("LOADFILE")){						//下面的代码为打开服务器数据库里的文件
		  ...
		  String viewDocHistory=MsgObj.GetMsgByName("viewDocHistory");  //是否查看留痕文件
		  ...
		  if(isDoc.equals("false")){
			...
			//判断留痕文件是否存在，如果已经存在生成正文，应该另存一个二次修改的文件
			if("chongqingshangtou".equals(SystemCommon.getCustomerName())){
				 if(com.js.util.util.JSFile.fileIsExists(mFilePath+"/upload/govdocumentmanager_his/"+mRecordID + mFileType)){
					 com.js.util.util.JSFile.copyToFileServerPath(myPath+ "/" + mRecordID + mFileType,mRecordID+"_2" + mFileType,
							 mFilePath+"/upload","/govdocumentmanager_his/");
				 }
			}
...
/*------------------------------------------------------*/
	   ...
	   //  }
         ...
         String moduleTypeStr=MsgObj.GetMsgByName("moduleType");//模块类型					//取得文档类型
          mFileSize=MsgObj.MsgFileSize();								//取得文档大小
          mFileDate=DbaObj.GetDateTime();								//取得文档时间
          mFileBody=MsgObj.MsgFileBody();
          String myPathStr="";
          System.out.println("moduleTypeStr:"+moduleTypeStr);
          if(moduleTypeStr.equals("govdocument")){
			myPathStr=mFilePath+"/upload/govdocumentmanager";
			MsgObj.MsgFileSave(myPathStr + "/" + mRecordID+"_wh" + mFileType);

         };													//如果保存为文件，则填写文件路径
          mDescript="定稿版本";	
          System.out.println("mUserName::"+mUserName);	
          System.out.println("myPath::"+myPathStr);	
           									//版本说明
          MsgObj.MsgTextClear();
