 ...
 * Generated at: 2023-01-06 09:58:30 UTC
...
public final class item_005fsubmit_005fback_jsp extends org.apache.jasper.runtime.HttpJspBase
...
//根据组织取所有用户
	...
	public void dealWithModule_15(String recordId) {
		...
		try {
			...
			stmt.executeUpdate("update OA_BOARDROOMAPPLY set STATUS=3 where BOARDROOMAPPLYID="
					+ recordId);
			stmt.executeQuery("delete from OA_BOARDROOM_MEETINGTIME where applyid="
					+ recordId + "");
			stmt.close();
	...
	public void dealWithModule_14(String recordId) {
		...
		try {
			conn = ds.getConnection();
			stmt = conn.createStatement();
			stmt.executeUpdate("update OA_EQUIPMENTAPPLY set STATUS=3 where EQUIPMENTAPPLYID="
					+ recordId);
			stmt.close();
		} catch (java.sql.SQLException e) {
			e.printStackTrace();
	...
	//处理入库退货流程
	public void dealWithModule_13_rk(String recordId, String userId,
			String userName) {
	...
	//处理出库退库流程
	public void dealWithModule_13_ck(String recordId, String userId,
			String userName) {
		IntoOutStockBD bd = new IntoOutStockBD();
	...
	public String getUserByGroup(String groupIdStr) {
		...
		try {
			...
			java.sql.ResultSet rs = stmt
					.executeQuery("SELECT DISTINCT EMP_ID FROM ORG_USER_GROUP WHERE GROUP_ID IN ("
							+ groupIdStr + ")");
			while (rs.next()) {
	...
	public String getWorkTitle(String processId, String recordId, String tableId) {
		...
		try {
			...
			if (module == 2) {
				//发文
				sql = "select documentsendfile_title from doc_documentsendfile where documentsendfile_id="
						+ recordId;
				...
				//收文
				sql = "select receivefile_title from doc_receivefile where receivefile_id="
						+ recordId;
				rs = stmt.executeQuery(sql);
				if (rs.next()) {
					workTitle = rs.getString(1);
  ...
  public void _jspService(final javax.servlet.http.HttpServletRequest request, final javax.servlet.http.HttpServletResponse response)
	...
	String backToActivityName = request
			.getParameter("backToActivityName");
	...
	if (request.getParameter("include_commField") != null) {
		dealwithMap.put("commentField",
				request.getParameter("include_commField"));
	}
	...
	String title = request.getParameter("titleFieldName").equals("") ? processName
			: request.getParameter(request
					.getParameter("titleFieldName"));
	String backInfo = submitPerson + "  " + submitTime + "   发起的"
			+ title + "被" + userName + "退回.<br>" + "退回原因：<br>"
			+ comment;
	...
	Map backMap = wfbBD.getBackToPerson(tableId, recordId, stepCount,
			initStepCount, workId);
	...
	if ("0".equals(backType)) {
		if ("11".equals(moduleId)) {
			...
			if (processName.indexOf("退货流程") >= 0
					|| processName.indexOf("进货流程") >= 0) { //入库、退货流程
	...
	workLogVO.setSendAction("退回" + com.js.oa.weixin.common.util.WorkflowForWeiXinUtil.decordStr(backToActivityName));
	//workLogVO.setReceiveUserId(mainTransactUser+"");
	workLogVO.setReceiveUserName(backToUserName == null ? ""
			: backToUserName);
	...
	if ("0".equals(backType)) {
		...
		if (!"".equals(to)) {
			...
			String url = "/jsoa/jsflow/item/jump_dealwith.jsp?status=-1&workId="
					+ empWorkId;
			com.js.system.service.messages.RemindUtil
					.sendMessageToUsers(title, url, to, "jsflow",
		...
		if (!"".equals(to)) {
			...
			String titleTemp = submitPerson + "发起的" + title + "被"
					+ userName + "退回到" + backToActivityName;

			for (int i = 0; i < toArr.length; i++) {
				com.js.system.service.messages.RemindUtil
						.sendMessageToUsers(titleTemp, url
								+ workIdArr[i], toArr[i], "jsflow",
								new java.util.Date(),
								new java.util.Date("2050/1/1"));
			}
		}
		//给发起人发通知
		title = "您发起的" + title + "被" + userName + "退回到"
				+ backToActivityName;
		to = backMap.get("submitEmpId").toString();
		com.js.system.service.messages.RemindUtil.sendMessageToUsers(
				title, url + backMap.get("submitWorkId").toString(),
				to, "jsflow", new java.util.Date(), new java.util.Date(
						"2050/1/1"));
...
if(SystemCommon.getCustomerName().equals("zcl")){
      ...
      out.write("\r\n\t\t}\r\n\t\t");
if(SystemCommon.getCustomerName().equals("zcl") || SystemCommon.getCustomerName().equals("ynnt")){
