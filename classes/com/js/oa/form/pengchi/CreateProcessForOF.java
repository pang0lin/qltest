...
public class CreateProcessForOF {
  ...
  public List<Map<String, String>> getFieldValueForOF() {
    ...
    try {
      ...
      if (ukids.length() > 0) {
        ...
        pstmt.executeBatch();
        conn.commit();
        System.out.println("------【成功修改OF（东莞）单表头的读取状态为230】-【UK主键】：" + ukids);
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("OFDG"))
    ...
    if (of != null && of.size() > 0 && ofmx != null && ofmx.size() > 0) {
      ...
      for (Map<String, String> ofmap : of) {
        ...
        if (!result.equals("-1") && result.length() < 10) {
          System.out.println("时间：[" + printTime + "]创建采购单OF（东莞）成功！--单号是：" + (String)ofmap.get("ddh"));
          continue;
        } 
        if (result.equals("-1")) {
          System.out.println("时间：[" + printTime + "]创建采购单OF（东莞）失败！--单号是：" + (String)ofmap.get("ddh"));
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("OFDG")) {
