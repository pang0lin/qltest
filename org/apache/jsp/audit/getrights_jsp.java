 ...
 * Generated at: 2023-01-06 10:00:43 UTC
...
if(request.getAttribute("rightList")!=null){
	rightClass=(List)request.getAttribute("rightClass");
      ...
      try {
        ...
        // /audit/getrights.jsp(40,0) name = id type = java.lang.String reqTime = false required = true fragment = false deferredValue = false expectedTypeName = null deferredMethod = false methodSignature = null
        _jspx_th_logic_005fiterate_005f0.setId("right");
        // /audit/getrights.jsp(40,0) name = name type = null reqTime = true required = false fragment = false deferredValue = false expectedTypeName = null deferredMethod = false methodSignature = null
        _jspx_th_logic_005fiterate_005f0.setName("rightList");
        // /audit/getrights.jsp(40,0) name = scope type = null reqTime = true required = false fragment = false deferredValue = false expectedTypeName = null deferredMethod = false methodSignature = null
...
boolean showAll;
String myRightId;
String key,className,oldClassName="";
    ...
    for(int i=0;i<myList.size();i++){
        ...
        if(myRight.getRightHasScope()!=0){
            ...
            if(selectRange.charAt(1)=='1'){
                //可以选择范围 "本人"
                out.println("<input type=\"radio\" name=\"rightScopeType"+num2+"\" value=\"1\" disabled ");
                if(oldRightId != null){
                    for( int j = 0; j < oldRightId.length; j ++){
                        if(oldRightId[j].equals(myRightId)){
                            if(oldRightType[j].equals("1")){
                                out.println("checked");
                                rowBegin=true;
                                break;
                            }
            ...
            if(selectRange.charAt(3)=='1'){
                //可以选择范围 "本组织"                
                out.println("<input type=\"radio\" name=\"rightScopeType"+num2+"\" value=\"3\" disabled ");
                if(oldRightId != null){
                    for( int j = 0; j < oldRightId.length; j ++){
                        if(oldRightId[j].equals(myRightId)){
                            if(oldRightType[j].equals("3")){
                                out.println("checked");
								rowBegin=true;
                                break;
                            }
            ...
            if(selectRange.charAt(2)=='1'){
                //可以选择范围 "本组织及下级组织"               
                out.println("<input type=\"radio\" name=\"rightScopeType"+num2+"\" value=\"2\" disabled ");
                if(oldRightId != null){
                    for( int j = 0; j < oldRightId.length; j ++){
                        if(oldRightId[j].equals(myRightId)){
                            if(oldRightType[j].equals("2")){
                                out.println("checked");
                                break;
                            }
                        }
            ...
            if(selectRange.charAt(0)=='1'){
                ...
                selectAllStr.append("<input type=\"radio\" name=\"rightScopeType").append(num2).append("\" value=\"0\" disabled ");
                if(oldRightId != null){
                    for( int j = 0; j < oldRightId.length; j ++){
                        if(oldRightId[j].equals(myRightId)){
                            if(oldRightType[j].equals("0")){
            ...
            if(selectRange.charAt(4)=='1'){
                //可以选择范围 "自定义"                
                out.println("<input type=\"radio\" name=\"rightScopeType"+num2+"\" value=\"4\" disabled ");                
                tmp="";
                tmpName="";
                if(oldRightId != null){
                    for( int j = 0; j < oldRightId.length; j ++){
            ...
            if(selectRange.charAt(4)=='1'){
                ...
                out.print(">&nbsp;&nbsp;");
                out.println("<input type=\"text\" disabled class=\"inputText\" name=\"rightScopeScopeDsp"+num2+"\" size=\"30\" id=\"rightScopeScopeDsp"+num2+"\" value='"+tmpName+"'  readonly onclick=\"javascript:selectEndowSC('rightScopeScope"+num2+"','rightScopeScopeDsp"+num2+"','org','','org','"+managerScope+"','yes');\">");
                
                out.println("<input type='hidden' name='rightScopeScope"+num2+"' value='"+tmp+"'>");
                out.println("<input type='hidden' name='rightdes"+num2+"' value='"+key+myRight.getRightName()+"'>");
                out.println("</td>");
            }
            num3++;
        }
