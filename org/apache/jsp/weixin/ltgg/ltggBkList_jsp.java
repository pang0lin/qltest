...
public final class ltggBkList_jsp extends org.apache.jasper.runtime.HttpJspBase
...
List<ForumPO> list = (List<ForumPO>) request.getAttribute("bbslist");
int size = Integer.valueOf(null!=request.getAttribute("size") ? request.getAttribute("size").toString() : "0");
...
String classId = null!=request.getAttribute("classId") ? request.getAttribute("classId").toString() : "";
String cardTitle = "论坛交流";
if("seeList".equals(action)) cardTitle = "我的关注";
      ...
      out.write("\" placeholder=\"请输入标题\" />\r\n\t\t   \t\t\t<div class=\"sousuo-11\" onclick=\"javascript:submitForm();\"><img src=\"images/weixin/ss.png\"></div>\r\n\t\t   \t\t</div>\r\n\t\t   \t</div>\r\n\t   \t</FORM>\r\n   \t\r\n\t   \t<!-- 信息列表 -->\r\n\t\t<div class=\"list\" style=\"overflow:auto;\">\r\n\t\t\t\t");
				...
				if(list!=null && list.size()>0){
					...
					for(int i=0;i<list.size();i++){
						ForumPO forumPO=list.get(i);
						String title=forumPO.getForumTitle();
						if(title.length() > 12){
							title = title.substring(0, 10) + "...";
						}
						String type = "";
						if(forumPO.getForumType() == 2){
							//type="&type=2";
						}
						String more = path + "/weiXinLtggAction.do?action=bbsInfo" + type + "&kit=yes&beginIndex=" 
								+ beginIndex + "&forumId=" + forumPO.getId()+"&operate="+action;
						String userAccount = StaticParam.getUserAccountsByEmpId(forumPO.getForumAuthorId()+"");
						
      out.write("\r\n\t\t\t\t        <a style=\"text-decoration: none;\"  href=\"");
      out.print(more);
      out.write("\">\r\n\t\t\t\t            <div class=\"item\">\r\n\t\t\t\t\t        \t<div class=\"picture\">\r\n\t\t\t\t             \t\t<img style=\"width:40px;height: 40px;\" border='0' src='/jsoa/weixin/common/getUserAvatar.jsp?userId=");
      out.print(userAccount);
      out.write("'/>\r\n\t\t\t\t             \t</div>\r\n\t\t\t\t\t\t\t\t<div class=\"content\">\r\n\t\t\t\t\t\t   \t\t\t<div class=\"first\">\r\n\t\t\t\t\t\t\t\t\t\t<div class=\"title\">");
      out.print(title );
      out.write("</div>\r\n\t\t\t\t\t\t\t\t\t\t<div class=\"time\">");
      out.print(format.format(forumPO.getForumIssueTime()).substring(5, 10) );
      out.write("</div>\r\n\t\t\t\t\t\t\t\t\t</div>\r\n\t\t\t\t\t\t\t\t\t<div class=\"second\">");
      out.print("论区："+forumPO.getForumClass().getClassName());
      out.write("</div>\r\n\t\t\t\t\t        \t</div>\r\n\t\t\t\t\t        </div>\r\n\t\t\t\t    \t</a>\r\n\t\t\t\t\t\t");
      ...
      out.write("\r\n\t\t</div>\r\n\t\t\r\n\t\t<!-- 分页 -->\r\n\t\t");
		...
		if (size > WapUtil.LIMITED) {
			if (upIndex >= 0){
				upUrl = path + "/weiXinLtggAction.do?action=" + action + "&amp;classId=" + classId + "&amp;beginIndex=" + upIndex;
			...
			if (nextIndex < size){
				nextUrl = path + "/weiXinLtggAction.do?action=" + action + "&amp;classId=" + classId + "&amp;beginIndex=" + nextIndex;
				if (null != keyword && !"".equals(keyword))
