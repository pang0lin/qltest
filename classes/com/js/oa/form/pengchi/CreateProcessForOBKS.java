...
public class CreateProcessForOBKS {
  ...
  public List<Map<String, String>> getFieldValueForOB() {
    ...
    try {
      ...
      if (ukids.length() > 0) {
        ...
        pstmt.executeBatch();
        conn.commit();
        System.out.println("------成功修改OB（昆山）单的读取状态！---UKID=" + ukids);
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("OBKS"))
    ...
    if (ob != null && ob.size() > 0 && obmx != null && obmx.size() > 0) {
      ...
      for (Map<String, String> obmap : ob) {
        ...
        if (!result.equals("-1") && result.length() < 10) {
          System.out.println("时间：[" + printTime + "]创建采购单OB（昆山）成功！--单号是：" + (String)obmap.get("ddh"));
          continue;
        } 
        if (result.equals("-1")) {
          System.out.println("时间：[" + printTime + "]创建采购单OB（昆山）失败！--单号是：" + (String)obmap.get("ddh"));
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("OBKS")) {
