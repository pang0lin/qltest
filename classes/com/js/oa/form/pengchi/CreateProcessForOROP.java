...
public class CreateProcessForOROP {
  ...
  public List<Map<String, String>> getFieldvalueForOPmx() {
    List<Map<String, String>> datas = new ArrayList<Map<String, String>>();
    String sql = "select KDUKID,KDDCTO,KDKCOO,KDDOCO,KDSFXO,KDLITM,KDMCU,KDAN8,KDTRDJ,KDVR01,KDDSC1,KDDSC2,DRDL01,KDPRP3,KDUOM,KDUOM2,KDPRRC,KDUORG,KDSQOR,KDURAT,CRPDTA.NTOD(KDDRQJ) KDDRQJ from PRODDTA.FE64311D@jdedblink,PRODCTL.F0005@jdedblink where  DRSY='41' and DRRT='P3' and DRKY=KDPRP3 and KDPOHP01=0 and KDPOHP02='N' and KDKCOO='00010' and KDOCTO='OR' and KDDCTO='OP'";
    DataSource ds = (new DataSourceBase()).getDataSource("jdbc/pengchi");
    Connection conn = null;
    try {
      conn = ds.getConnection();
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("OROPDG"))
        info = formBaseInfo.get(i); 
    } 
    if (op != null && op.size() > 0 && opmx != null && opmx.size() > 0) {
      String result = "-1";
      for (Map<String, String> opmap : op) {
        ...
        try {
          ...
          String fqr = ((String)opmap.get("zdr")).trim();
          String jsr = "";
          if (fqr.equals(info.get("submitNameZjb"))) {
            jsr = info.get("receiveNameZjb");
          } else {
          ...
          xml.append("<WorkFlow>");
          if (fqr.equals(info.get("submitNameZjb"))) {
            xml.append("<Process processId=\"").append(info.get("processIdZjb")).append("\"/>");
          } else {
          ...
          for (Map<String, String> mxmap : opmx) {
            if (((String)opmap.get("ukzj")).equals(mxmap.get("ukzj")) && ((String)opmap.get("ddlx")).equals(mxmap.get("ddlx")) && ((String)opmap.get("ddgs")).equals(mxmap.get("ddgs")) && ((String)opmap.get("ddh")).equals(mxmap.get("ddh")) && ((String)opmap.get("ddhz")).equals(mxmap.get("ddhz"))) {
              ...
              xml.append("<field name=\"kf_cgdoropmx_f6357\" type=\"varchar\">").append(((String)mxmap.get("ckj")).trim()).append("</field>");
          ...
          if (flowxml.indexOf("SubTable") > 0) {
            ...
            if (!result.equals("-1") && result.length() < 10) {
              System.out.println("时间：[" + printTime + "]创建采购单OROP(东莞)成功！--单号是：" + (String)opmap.get("ddh"));
              continue;
            } 
            if (result.equals("-1")) {
              System.out.println("时间：[" + printTime + "]创建采购单OROP(东莞)失败！--单号是：" + (String)opmap.get("ddh"));
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("OROPDG")) {
          ...
          nodeMap.put("processIdZjb", node.getAttributeValue("processIdZjb"));
          nodeMap.put("submitNameZjb", node.getAttributeValue("submitNameZjb"));
          nodeMap.put("receiveName", node.getAttributeValue("receiveName"));
          nodeMap.put("receiveNameZjb", node.getAttributeValue("receiveNameZjb"));
