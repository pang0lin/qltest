...
public class CreateProcessForOS {
  ...
  public List<Map<String, String>> getFieldValueForOS() {
    ...
    String sql = "select THUKID,THKCOO,THDCTO,THSFXO,THDOCO,CRPDTA.NTOD(THTRDJ) THTRDJ,THDL07,THVR01,THRMK from  PRODDTA.FE64319H@jdedblink where THDCTO='OS' and THNXTR=' ' and THPOHP01=0 and THKCOO='00010'";
  ...
  public List<Map<String, String>> getFieldvalueForOSmx() {
    List<Map<String, String>> datas = new ArrayList<Map<String, String>>();
    String sql = "select PDASID,PDDL04,PDDL01,PDACL2,PDDS50,PDDL03,PDUOM,PDUORG,PDVR01,CRPDTA.NTOD(PDDRQJ) PDDRQJ,PDURAT,PDUKID,PDDCTO,PDKCOO,PDDOCO,PDSFXO from PRODDTA.FE64319D@jdedblink where PDPOHP01=0 and PDKCOO='00010' and PDDCTO=?";
    DataSource ds = (new DataSourceBase()).getDataSource("jdbc/pengchi");
    Connection conn = null;
    try {
      conn = ds.getConnection();
  ...
  public void startProcess() {
    ...
    for (int i = 0; i < formBaseInfo.size(); i++) {
      if (((String)((Map)formBaseInfo.get(i)).get("formid")).equals("OSDG"))
    ...
    if (os != null && os.size() > 0 && osmx != null && osmx.size() > 0) {
      ...
      for (Map<String, String> osmap : os) {
        ...
        if (!result.equals("-1") && result.length() < 10) {
          System.out.println("时间：[" + printTime + "]东莞-创建请修单OS成功！--单号是：" + (String)osmap.get("ddh"));
          continue;
        } 
        if (result.equals("-1")) {
          System.out.println("时间：[" + printTime + "]东莞-创建请修单OS失败！--单号是：" + (String)osmap.get("ddh"));
  ...
  public List<Map<String, String>> getBaseinfo() {
    ...
    try {
      ...
      for (int i = 0; i < formNodes.size(); i++) {
        ...
        if (((String)nodeMap.get("formid")).equals("OSDG")) {
