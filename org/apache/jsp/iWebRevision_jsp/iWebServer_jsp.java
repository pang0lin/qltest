...
public final class iWebServer_jsp extends org.apache.jasper.runtime.HttpJspBase
...
public String groupIds = ""; 
  ...
  public class iWebRevision {
    ...
    private String isdm=com.js.util.config.SystemCommon.getIsdm();
  ...
  //读取大字段信息 达梦数据库
    private void GetAtDmBlob(java.sql.Blob vField, int vSize) throws IOException {
      try {
        mFileBody = new byte[vSize];
        InputStream instream = vField.getBinaryStream();
        instream.read(mFileBody, 0, vSize);
    ...
    private boolean SaveHistory() {
      ...
      if(databaseType.indexOf("oracle")>=0){
    	  Sql = "insert into Document_History (ID,RecordID,FieldName,MarkName,UserName,DateTime,HostName,MarkGuid) values (?,?,?,?,?,?,?,?) ";
    	  iID = DbaObj.GetMaxID("Document_History", "ID");
      }else{
    	  Sql = "insert into Document_History (RecordID,FieldName,MarkName,UserName,DateTime,HostName,MarkGuid) values (?,?,?,?,?,?,?) ";
      }
    ...
    private boolean SignatureImage(String vMarkName, String vPassWord) {
      String Sql;
      if(databaseType.indexOf("oracle")>=0){
    	  Sql= "SELECT MarkBody,MarkType,MarkSize FROM Signature WHERE MarkName='" + vMarkName + "' and PASSWORD='" + vPassWord + "'";
      }else{
    	  Sql = "SELECT MarkBody,MarkType FROM Signature WHERE MarkName='" + vMarkName + "' and PassWord='" + vPassWord + "'";
      }
      boolean mResult = false;
      if (DbaObj.OpenConnection()) {
        try {
          ...
          if (result.next()) {
            ...
            if(databaseType.indexOf("oracle")>=0){
	            ...
	            if("1".equals(isdm)){
	        	   GetAtDmBlob(result.getBlob("MarkBody"), mFileSize);
	            }else{
    ...
    private boolean SaveSignature() {
      boolean mResult = false;
      if(mFieldName.indexOf("_SendOut")>0){
    	  mFieldName="SendOut";
      }
      if (DbaObj.OpenConnection()) {
        java.sql.PreparedStatement prestmt = null;
        try {
          ...
          int iID=-1;
          if(databaseType.indexOf("oracle")>=0){
        	  Sql = "insert into Document_Signature (ID,RecordID,FieldName,FileBody,FileSize,UserName,DateTime,HostName) values (?,?,?,EMPTY_BLOB(),?,?,?,?) ";
              iID = DbaObj.GetMaxID("Document_Signature", "ID");
              mFileBody = mFieldValue.getBytes();
              mFileSize = mFileBody.length;
              DbaObj.Conn.setAutoCommit(false);
          }else{
        	  Sql = "insert into Document_Signature (RecordID,FieldName,UserName,DateTime,HostName,FieldValue) values (?,?,?,?,?,? ) ";
          }
          ...
          if(databaseType.indexOf("oracle")>=0){
	          ...
	          if("1".equals(isdm)){
	        	  ...
	        	  InfoUtil.insert_oracle_blob("Document_Signature", "FileBody", "ID", (long)iID, mFileBody);	        	  
      ...
      if (DbaObj.OpenConnection()) {
        ...
        try {
             ...
             if(databaseType.indexOf("oracle")>=0){
		          mFileBody = mFieldValue.getBytes();
		          mFileSize = mFileBody.length;
		          Sql = "update Document_Signature Set UserName=?,DateTime=?,HostName=?,FileBody=EMPTY_BLOB(),FileSize=? where RecordID = '" + mRecordID + "' and FieldName='" + mFieldName + "'";
		          DbaObj.Conn.setAutoCommit(false);
             }else{
            	 Sql = "update Document_Signature Set UserName=?,DateTime=?,HostName=?,FieldValue=? where RecordID = ? and FieldName=?";
             }
             prestmt = DbaObj.Conn.prepareStatement(Sql);
    ...
    private boolean ShowSignatureIS() {
      ...
      String Sql;
      if(databaseType.indexOf("oracle")>=0){   
    	  Sql= "SELECT 1 FROM Document_Signature WHERE RecordID='" + mRecordID + "' and FieldName='" + mFieldName + "'";
      }else{
    	  Sql = "SELECT * FROM Document_Signature WHERE RecordID='" + mRecordID + "' and FieldName='" + mFieldName + "'";
      }
      if (DbaObj.OpenConnection()) {
        try {
          ResultSet result = DbaObj.ExecuteQuery(Sql);
    ...
    private boolean LoadSignature() {
      ...
      String Sql;
      if(databaseType.indexOf("oracle")>=0){   
    	  Sql= "SELECT FileBody,FileSize FROM Document_Signature WHERE RecordID='" + mRecordID + "' and FieldName='" + mFieldName + "'";
      }else{
    	  Sql = "SELECT FieldValue FROM Document_Signature WHERE RecordID='" + mRecordID + "' and FieldName='" + mFieldName + "'";
      }
        ...
        try {
          ...
          if (result.next()) {
        	  if(databaseType.indexOf("oracle")>=0){  
		           ...
		           if("1".equals(isdm)){
		        	   GetAtDmBlob(result.getBlob("FileBody"), mFileSize);
		           }else{
