...
public final class item_005fsubmit_005fcomplete_jsp extends org.apache.jasper.runtime.HttpJspBase
...
//根据组织取所有用户
	...
	public void dealWithModule_15(String recordId) {
		...
		try {
			...
			String motif="";
			String emcee="",attendeeEmpId="",nonvotingEmpId="",notePersonId="",attendeeLeaderId="";
			java.util.Date meetingDate=null;
			String beginTime="",endTime="";
			
			
			String sql="SELECT ob.boardroomId,ob.name,rapply.motif,emcee,rapply.attendeeEmpId,rapply.nonvotingEmpId,rapply.attendeeLeaderId,rapply.NOTEPERSONID FROM oa_boardroomapply rapply LEFT JOIN oa_boardroom ob ON rapply.boardroomId=ob.boardroomid where rapply.boardroomapplyid=" + recordId;
			java.sql.ResultSet rs=stmt.executeQuery(sql);
			if(rs.next()){
				boardroomId=rs.getString(1);
				boardRoomName=rs.getString(2);
				motif=rs.getString(3);
				emcee=rs.getString(4);
				attendeeEmpId=rs.getString(5);
				nonvotingEmpId=rs.getString(6);
				attendeeLeaderId=rs.getString(7);
				notePersonId=rs.getString(8);				
			}
			rs.close();
			
			sql="SELECT meetingtime,starttime,endtime FROM oa_boardroom_meetingtime WHERE applyid="+recordId;
			rs=stmt.executeQuery(sql);
			if(rs.next()){
				meetingDate=rs.getDate(1);
            ...
            int destineDateBeginTime = Integer.parseInt(beginTime);
            int destineDateEndTime = Integer.parseInt(endTime);
            String tixing = fomatter.format(meetingDate);
            
            String _mailsubject = tixing +(destineDateBeginTime / 3600) + ":" +
			...
			String userIdStr = attendeeEmpId==null?"":attendeeEmpId;
			...
			String s1 = "";
			if (nonvotingEmpId != null &&
				!"".equals(nonvotingEmpId) &&
				!"null".equals(nonvotingEmpId)) {
				nonvotingEmpId = "$" +nonvotingEmpId +"$";
			...
			String s2 = "";
			if (attendeeLeaderId != null &&
				!"".equals(attendeeLeaderId) &&
				!"null".equals(attendeeLeaderId)) {
				attendeeLeaderId = "$" +attendeeLeaderId +"$";
			...
			String s3 = "";
			if (notePersonId!=null&&
					!"".equals(notePersonId)
					&&!"null".equalsIgnoreCase(notePersonId)) {
				notePersonId = "$" +notePersonId +"$";
			...
			String _userIdStr = userIdStr + s0 + s1 + s2+s3 + "-1";
			...
			String link = "BoardRoomAction.do?action=selectBoardroomApplyView&boardroomApplyId="+recordId+"&boardroomName="+boardRoomName+"&type=view&executeStatus=false";
					
			//发送通知	
			if(!"-1".equals(_userIdStr)){
				com.js.system.service.messages.RemindUtil.sendMessageToUsers(_mailsubject, link, _userIdStr, "Meeting", new java.util.Date(), new java.util.Date("2050/1/1"));
			}
	...
	private Object[] setPara(String recordId, String mode) {
		...
		try {
			...
			String sql = "select goods_id goodsid,ss_amount amount,price,goods_money money,goods_name goodsname,goods_unit goodsunit,goods_specs from st_ssdetail where ss_no="
					+ recordId
					+ " union all "
					+ "select goods_id goodsid,amount amount,mcost price,goods_money money,goods_name goodsname,goods_unit goodsunit,goods_specs from st_ptdetail where pt_no="
					+ recordId;
			rs = stmt.executeQuery(sql);
			while (rs.next()) {
			...
			sql = "select ss_stock stock from st_ssmaster where ss_no=" + recordId
					+ " union all select pt_stock stock from st_ptmaster where pt_no=" + recordId;
			rs = stmt.executeQuery(sql);
			if (rs.next()) {
				stockIdS[0] = rs.getString("stock");
			}
			stmt.close();
			Object[] para = { goodsId, amount, price, money, goodsName, goodsUnit, goodsSpecs, stockIdS };
			return para;
	...
	/**
	 * 发文办理结束后 更新 DOC_DOCUMENTSENDFILE的 sendfile_transactStatus='1'
	 * @param groupIdStr String
	 * @return String
	 */
	public void updateDocmentsendfile(String recordId) {
		int module = 1;
		String workTitle = "";
		com.js.util.util.DataSourceBase dsb = new com.js.util.util.DataSourceBase();
		javax.sql.DataSource ds = dsb.getDataSource();
		...
		try {
			conn = ds.getConnection();
			stmt = conn.createStatement();
			String sql = "update DOC_DOCUMENTSENDFILE c set c.sendfile_transactStatus='1' where c.DOCUMENTSENDFILE_ID="+recordId;
	...
	/**
	 * 收文办理结束后 更新 doc_receivefile的 RECEIVEFILE_STATUS='1'
	 * @param groupIdStr String
	 * @return String
	 */
	public void updateDocmentrecfile(String recordId) {
		int module = 1;
		String workTitle = "";
		com.js.util.util.DataSourceBase dsb = new com.js.util.util.DataSourceBase();
		javax.sql.DataSource ds = dsb.getDataSource();
		...
		try {
			conn = ds.getConnection();
			stmt = conn.createStatement();
			String sql = "update doc_receivefile v set v.receivefile_status='1' where v.RECEIVEFILE_ID="+recordId;
			stmt.executeUpdate(sql);
	...
	public void infoArchives(String processeId,String recordId){
	      ...
	      try{
	      	...
	      	if(rs.next()){
	      		...
	      		if(rs.getString(1)==null||"".equals(rs.getString(1))||"null".equalsIgnoreCase(rs.getString(1))){}else{
	      			flag = true;
	      		}
	      	}
	      	rs.close();
	      	if(flag){
	      		/*SELECT emp.empname,emp.emp_id,o.org_id,o.orgName,o.orgIdString,j.receivefile_sendfileunit 
	      		FROM doc_receivefile j JOIN org_employee emp ON j.createdemp=emp.emp_id JOIN org_organization_user e 
	      		ON emp.emp_id=e.EMP_ID JOIN org_organization o ON e.ORG_ID=o.org_id WHERE j.receivefile_id=*/
		        	sql = "SELECT emp.empname,emp.emp_id,o.org_id,o.orgName,o.orgIdString,j.receivefile_sendfileunit,receivefile_filenumber,receivefile_title " +
		        			" FROM jsf_work w JOIN doc_receivefile j ON w.WORKRECORD_ID=j.receivefile_id " +
		        			" JOIN org_employee emp ON w.WF_CUREMPLOYEE_ID=emp.EMP_ID " +
		        			" JOIN org_organization_user e ON emp.emp_id=e.EMP_ID " +
		        			" JOIN org_organization o ON e.ORG_ID=o.org_id " +
		        			" WHERE w.WORKPROCESS_ID="+processeId+" AND w.WORKRECORD_ID=" +recordId+
		        	...
		        	new InfoArchives().saveInfo(list, list.get(3).toString(),"GWGL-SWGL");
		        }
	      	base.end();
	      }catch(Exception e){
	      	base.end();
    ...
    public void infoArchivesSend(String processeId,String recordId){
  	  DataSourceBase base = new DataSourceBase();
        ResultSet rs = null;
        List<String> list = new ArrayList<String>();
        boolean flag = false;
        try{
      	  	int sendFile = 0;
				base.begin();
				String sql = "SELECT infoChannelId,WORKFLOWPROCESSNAME,sendFileType FROM jsf_workflowprocess " +
						"WHERE WF_WORKFLOWPROCESS_ID="+processeId;
				rs = base.executeQuery(sql);
				if(rs.next()){
					list.add(rs.getString(2)+"");
					list.add(rs.getString(1));
				...
				rs.close();
				if(flag){
					/*SELECT emp.empname,emp.emp_id,o.org_id,o.orgName,o.orgIdString,j.documentsendfile_title FROM 
					doc_documentsendfile j JOIN org_employee emp ON j.createdemp=emp.emp_id JOIN org_organization_user e 
					ON emp.emp_id=e.EMP_ID JOIN org_organization o ON e.ORG_ID=o.org_id WHERE j.documentsendfile_id=*/
					sql = "SELECT emp.empname,emp.emp_id,o.org_id,o.orgName,o.orgIdString,j.documentsendfile_title,DOCUMENTSENDFILE_BYTENUMBER " +
							" FROM jsf_work w JOIN doc_documentsendfile j ON w.WORKRECORD_ID=j.documentsendfile_id " +
							" JOIN org_employee emp ON w.WF_CUREMPLOYEE_ID=emp.EMP_ID " +
							" JOIN org_organization_user e ON emp.emp_id=e.EMP_ID " +
							" JOIN org_organization o ON e.ORG_ID=o.org_id " +
							" where w.WORKPROCESS_ID="+processeId+" AND w.WORKRECORD_ID=" +recordId+
					...
					list.add(recordId);
					if(sendFile==0){
						new InfoArchives().saveInfo(list, list.get(3).toString(),"GWGL-FWGL");
					}else{
						new InfoArchives().saveInfo(list, list.get(3).toString(),"GWGL-DOC");
					}
				}
				base.end();
        }catch(Exception e){
  ...
  static {
    ...
    _jspx_imports_classes.add("com.js.oa.info.util.InfoArchives");
  ...
  public void _jspService(final javax.servlet.http.HttpServletRequest request, final javax.servlet.http.HttpServletResponse response)
	...
	String[] para = { tableId, recordId, curActivityId, processName, submitPerson, remindFieldValue, curTransactType, stepCount,
					isStandForWork, userId, standForUserId, standForUserName, docTitle };
	...
	if (result.intValue() == 1) {
		...
		//整个流程结束2016-8.30
        Map formClassNameMethod2 = workFlowCommonBD.getProcessClassMethod(request.getParameter("processId"));
        String formClassName2 = "";
        String formClassMethod2 = "";
        if(formClassNameMethod2 != null && formClassNameMethod2.get("formClassName") != null && formClassNameMethod2.get("formClassCompMethod") != null){
            formClassName2 = formClassNameMethod2.get("formClassName").toString();
		...
		if ("2".equals(moduleId)) {//发文结束 后 更新 DOC_DOCUMENTSENDFILE的  sendfile_transactStatus='1'
			updateDocmentsendfile(recordId);
			if(com.js.util.config.SystemCommon.getArchiveToInfo()==1){
			  String processIdc = request.getParameter("processId");
			  this.infoArchivesSend(processIdc, recordId);
			}
		}
		if ("3".equals(moduleId)) {//收文办理结束后 更新 doc_receivefile的 RECEIVEFILE_STATUS='1'
			updateDocmentrecfile(recordId);
			 if(com.js.util.config.SystemCommon.getArchiveToInfo()==1){
		        	//收文归档到知识中
			        String processIdc = request.getParameter("processId");
			        this.infoArchives(processIdc, recordId);
		     }
		}
