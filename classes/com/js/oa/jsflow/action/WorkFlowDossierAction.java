...
public class WorkFlowDossierAction extends Action {
  ...
  public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
    ...
    if (type.equals("catalog")) {
      ...
      httpServletRequest.setAttribute("searchStatus", httpServletRequest.getParameter("searchStatus"));
      httpServletRequest.setAttribute("queryType", httpServletRequest.getParameter("queryType"));
      ...
      if (formTable != null && formTable.length > 0) {
        ...
        if (httpServletRequest.getParameter("pager.offset") == null || httpServletRequest.getParameter("pager.offset").toUpperCase().equals("NULL")) {
          paraSQL = getQueryDataSQL(httpServletRequest, formTable[0][0], formTable[0][1]);
        ...
        if (paraSQL != null) {
          ...
          String para = paraSQL.substring(paraSQL.indexOf("select") + 7, paraSQL.indexOf(" from "));
          String fromTable = paraSQL.substring(paraSQL.indexOf(" from ") + 6, paraSQL.indexOf(" where "));
          String where = paraSQL.substring(paraSQL.indexOf(" where "));
  ...
  public String[][] getQueryData(HttpServletRequest request, String tableId, String tableName) {
        ...
        if ("1".equals(request.getParameter(String.valueOf(queryFields[i][2]) + "date"))) {
          ...
          if ("date".equals(paras[i][1])) {
            ...
            try {
              ...
              start = format.format(format.parse(String.valueOf(start) + ":00"));
              end = format.format(format.parse(String.valueOf(end) + ":00"));
            } catch (Exception exx) {
              exx.printStackTrace();
            } 
            if (DbOpt.dbtype.indexOf("oracle") >= 0) {
              paras[i][2] = String.valueOf(paras[i][0]) + " is not null and " + paras[i][0] + " between '" + start + "' and '" + end + "'";
            } else {
              paras[i][2] = String.valueOf(paras[i][0]) + " between '" + start + "' and '" + end + "' ";
            } 
          } 
    ...
    String para = "";
    parameters = String.valueOf(parameters) + "workdonewithdate,WORK_HANGUP,worksubmittime,workstatus,workprocess_id,WORKCURSTEP,WF_CUREMPLOYEE_ID,WORKRECORD_ID,WORKFILETYPE,WF_WORK_ID from " + tableStr;
    ...
    if ("QueryData".equals(request.getParameter("type"))) {
      request.getSession(true).setAttribute("sql", parameters);
    } else {
      request.getSession(true).removeAttribute("sql");
    } 
    request.getSession(true).setAttribute("js", js);
    String[][] result = (new CustomFormBD()).loadDataBySQL(parameters, String.valueOf(fieldNum + 10));
  ...
  private boolean hasAdminRights(HttpServletRequest request) {
    ...
    try {
      ...
      String adminUser = (map == null) ? null : ((map.get("processadminperson") == null) ? null : map.get("processadminperson").toString());
      String adminGroup = (map == null) ? null : ((map.get("processadmingroup") == null) ? null : map.get("processadmingroup").toString());
      String adminOrg = (map == null) ? null : ((map.get("processadminorg") == null) ? null : map.get("processadminorg").toString());
  ...
  public String[][] getQueryDataAndSub(HttpServletRequest request, String tableId, String tableName) {
    ...
    request.getSession(true).setAttribute("js", js);
    String[][] result = (new CustomFormBD()).loadDataBySQL(parameters, String.valueOf(listFields.length + subListFields.length + 10));
    if (result == null || result.length < 1)
      result = new String[0][0]; 
    return result;
  }
  ...
  public String getQueryDataSQL(HttpServletRequest request, String tableId, String tableName) {
    CustomDatabaseBD bd = new CustomDatabaseBD();
    String[][] queryFields = bd.getQueryField(tableId);
    ...
    parameters = String.valueOf(parameters) + request.getParameter("processId") + " and (" + rightWhere + ") ";
    if (paras != null && paras.length > 0)
      for (int j = 0; j < paras.length; j++) {
        if (paras[j][2] != null && paras[j][2].length() > 0) {
          parameters = String.valueOf(parameters) + " and " + paras[j][2];
          para = String.valueOf(para) + paras[j][2] + " and ";
        } 
      }  
    request.getSession(true).setAttribute("para", para);
    ...
    if (searchStatus == null || "null".equals(searchStatus))
      searchStatus = ""; 
    if ("0".equals(request.getParameter("queryType"))) {
      parameters = String.valueOf(parameters) + " and WORKSTATUS=1";
      if (!"".equals(searchStatus))
        parameters = String.valueOf(parameters) + " and workcurstep like '%" + searchStatus + "%'"; 
    } else if ("100".equals(request.getParameter("queryType"))) {
      if ("".equals(searchStatus)) {
        parameters = String.valueOf(parameters) + " and (WORKSTATUS=100)";
      } else {
        parameters = String.valueOf(parameters) + " and workstatus=" + searchStatus;
      } 
    } 
    ...
    if ("1".equals(request.getParameter("completeDate"))) {
      js = String.valueOf(js) + "document.all.completeDate.checked=true;";
      js = String.valueOf(js) + "document.all.completeStartDate.value='" + request.getParameter("completeStartDate") + "';";
      js = String.valueOf(js) + "document.all.completeEndDate.value='" + request.getParameter("completeEndDate") + "';";
      if (DbOpt.dbtype.indexOf("oracle") >= 0) {
        parameters = String.valueOf(parameters) + " and workdonewithdate between to_date('" + request.getParameter("completeStartDate") + "','YYYY-MM-DD') and to_date('" + request.getParameter("completeEndDate") + "','YYYY-MM-DD') ";
      } else if (DbOpt.dbtype.indexOf("sqlserver") >= 0) {
        parameters = String.valueOf(parameters) + " and workdonewithdate>= convert(datetime,'" + request.getParameter("completeStartDate") + "') and " + tableName + "_date<=convert(datetime,'" + request.getParameter("completeEndDate") + " 23:59:59') ";
      } else if (DbOpt.dbtype.indexOf("mysql") >= 0) {
        parameters = String.valueOf(parameters) + " and workdonewithdate>= date_format('" + request.getParameter("completeStartDate") + "','%Y-%m-%d') and " + tableName + "_date<=date_format('" + request.getParameter("completeEndDate") + " 23:59:59','%Y-%m-%d %H:%i:%s') ";
      } 
    } 
    String order = (request.getParameter("orderField") == null || request.getParameter("orderField").trim().equalsIgnoreCase("null")) ? "WF_WORK_ID " : request.getParameter("orderField");
    order = String.valueOf(order) + " " + ((request.getParameter("orderType") == null || request.getParameter("orderType").trim().equalsIgnoreCase("null")) ? " desc  " : request.getParameter("orderType"));
    if (parameters.endsWith("and "))
