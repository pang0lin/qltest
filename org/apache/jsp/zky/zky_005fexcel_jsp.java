...
public final class zky_005fexcel_jsp extends org.apache.jasper.runtime.HttpJspBase
...
try{
    ...
    WritableSheet ws = wwb.createSheet("绩效等级", 10);
    ws.setRowView(0, 500);//设置第一行高度
    ws.setColumnView(0, 10);//设置第一列宽度
    ...
    //插入单元格值（列，行，值，单元格样式）
    ws.addCell(new Label(0, 0, "中科院绩效等级统计", fwcf));
    ws.addCell(new Label(0, 1, "姓名", fwcf));
    ws.addCell(new Label(1, 1, "工号", fwcf));
    ws.addCell(new Label(2, 1, "得分", fwcf));
    ws.addCell(new Label(3, 1, "等级", fwcf));
    for(int i=0;i<list.size();i++){
    	String[] obj = list.get(i);
    	for(int y=0;y<4;y++){
    		ws.addCell(new Label(y,i+2,obj[y]+"",wcf));
    	}
    }
