 ...
 * Generated at: 2023-01-06 09:53:40 UTC
...
if("group".equals(type)){
	...
	if("".equals(range) || range.indexOf("*")>=0){
		...
		if(databaseType.indexOf("mysql")>=0){
			rs=stmt.executeQuery("select orgidstring from org_organization where '"+range+"' like concat('%*',org_id,'*%') and domain_id=" + domainId);
		}else{
			rs=stmt.executeQuery("select orgidstring from org_organization where '"+range+"' like jsdb.FN_LINKCHAR(jsdb.FN_LINKCHAR('%*',org_id),'*%') and domain_id=" + domainId);
		}
