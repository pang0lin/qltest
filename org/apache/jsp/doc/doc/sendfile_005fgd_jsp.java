...
public final class sendfile_005fgd_jsp extends org.apache.jasper.runtime.HttpJspBase
...
if(request.getParameter("gd") != null){
    ...
    //pageContent = pageContent.replace("id=popToolbar", "id=popToolbar style='display:none'");
    if("rws".equals(SystemCommon.getCustomerName())){
    	pageContent = pageContent.replace("id=popToolbar", "id=popToolbar style='display:none'");
    }
    
    pageContent=pageContent.replace("id=panleAll", "id=panleAll style='display:none'");
    pageContent=pageContent.replace(",DocToArchives", "");
	...
	String s1=pageContent.substring(0,pos+21)+"Close,Viewtext\">";
    ...
    new com.js.oa.archives.action.ArchivesAction().addArchivesWaitPigeonhole(request.getParameter("fileTitle"),tmp,Long.valueOf(request.getParameter("fileId")),"GWGL",session.getAttribute("userName").toString(), new java.util.Date(), returnValue, request, createdEmp, createdOrg,"FWGL");
    ...
    content.append("<html><head><meta charset=\"GB2312\"/><title>发文归档</title></head>");
    content.append("<script src=\"/jsoa/js/util.js\"></script>");
    ...
    if("rws".equals(SystemCommon.getCustomerName())){
      new TongBuService().tongbuForSendDoc(request.getParameter("fileId"), fileName,session.getAttribute("userId").toString());
    }
  	/*//将html代码转化成word
