...
public class WorkflowForOBOP extends Workflow {
  public String complete(HttpServletRequest request) {
    String result = super.complete(request);
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
    String recordId = request.getParameter("recordId");
    String comment = request.getParameter("include_comment");
    String sql = "Select ukzj,shjg,ddgs,ddh,ddlx,ddhz From kf_cgdobop Where kf_cgdobop_id=?";
    ...
    String sql1 = "Select ukzj From kf_cgdobopmx Where kf_cgdobopmx_foreignkey=?";
  ...
  public void back(HttpServletRequest request) {
    ...
    String sql = "Select ukzj,ddgs,ddh,ddlx,ddhz From kf_cgdobop Where kf_cgdobop_id=?";
    DataSourceBase db = new DataSourceBase();
    DataSource ds = db.getDataSource();
    Connection conn = null;
    PreparedStatement pstmt = null;
    ...
    try {
      ...
      rs.close();
      pstmt.close();
      String thsql = "update  kf_cgdobop set thbz=? where kf_cgdobop_id=?";
    ...
    try {
      ...
      pstmt2.close();
      if (resultInt > 0) {
        System.out.println("-----中间表头(OP)数据退回：回写成功--订单号:OBOP【" + opddh + "】");
      } else {
        System.out.println("-----中间表头(OP)数据退回：回写失败--订单号:OBOP【" + opddh + "】");
      } 
      conn2.close();
    } catch (Exception e) {
      if (conn2 != null)
