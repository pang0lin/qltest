...
public final class statisticsDataExport_jsp extends org.apache.jasper.runtime.HttpJspBase
...
try{ 	   
    ...
    String rightType=request.getAttribute("rightType").toString();
    String valueType=request.getAttribute("valueType").toString();
    
    String fileName="数据（"+rightType+"）分析.xls";
    fileName=new String(fileName.getBytes("GBK"),"iso-8859-1");  
    
    /*response.setContentType("application/OCTET-STREAM;charset=GBK");
    response.setHeader("Content-disposition", "attachment; filename="+"数据表-数据（"+rightType+"）分析.xls");// 设定输出文件头   
    ...
    //创建一个工作表    
    jxl.write.WritableSheet ws = wwb.createSheet(rightType, 10);
    ws.setRowView(0, 500);
	...
	ws.addCell(new jxl.write.Label(1, 0,"统计对象", wcf));
	ws.addCell(new jxl.write.Label(2, 0,valueType, wcf));
    //设置内容
    ...
    float  sum=0;
   	if(null!=dataList&&dataList.size()>0){
   		for(int w=0;w<dataList.size();w++){
			...
			for(int k=0;k<2;k++){
					...
					if(k==1){
						sum+=Float.parseFloat(null==obj[k]?"0":obj[k].toString());
					}
			}
			rowCount=w;
   		}
   	...
   	rowCount=rowCount+1;
		...
		ws.addCell(new jxl.write.Label(2, rowCount+1,String.valueOf(fnum.format(sum)), wcf));
    wwb.write();
    wwb.close(); 	  
